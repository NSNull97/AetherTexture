...
final class AetherPageViewControllerTests: XCTestCase {
    ...
    func testEmbeddedPageScrollUpdatesHostingNavigationBarSeparator() throws {
        let parentData = AetherAppearance.withRuntimeCurrent(.liquidGlassV2) {
            NavigationBarPresentationData.aetherAppearance()
        }
        let childData = AetherAppearance.withRuntimeCurrent(.liquidGlassV2) {
            NavigationBarPresentationData.aetherAppearance()
        }
        ...
        XCTAssertEqual(childBar.stripeNode.alpha, 0.0)
        XCTAssertEqual(hostingBar.stripeNode.alpha, 1.0)
    }
    ...
    func testScrollProgressMovesPagerLensContinuously() throws {
        ...
        let scrollView = controller.backingScrollViewForTesting
        scrollView.bounds = CGRect(x: 0.0, y: 0.0, width: 320.0, height: 480.0)
        scrollView.contentOffset = CGPoint(x: 160.0, y: 0.0)
    ...
    func testSegmentedControlKeepsAtLeastTwelvePointsOfHorizontalPaddingPerSegment() throws {
        ...
        XCTAssertNotNil(control.descendantTextView(text: "A"))
        let titleWidth = ("A" as NSString).size(
            withAttributes: [.font: UIFont.systemFont(ofSize: 14, weight: .semibold)]
        ).width
        XCTAssertGreaterThanOrEqual(firstFrame.width, titleWidth + 24.0 - 0.5)
    }

    func testSegmentedControlCentersTitleFramesVertically() throws {
        let control = AetherSegmentedControl(
            items: [
                AetherSegmentedControl.Item(title: "Посты"),
                AetherSegmentedControl.Item(title: "Медиа"),
                AetherSegmentedControl.Item(title: "Нравится")
            ],
            selectedIndex: 0
        )
        control.frame = CGRect(x: 0.0, y: 0.0, width: 320.0, height: 36.0)
        ...
        let titleViews = control.descendantViews(accessibilityLabel: "Посты")
        XCTAssertTrue(titleViews.contains { view in
            view.frame.height < control.bounds.height - 8.0
                && abs(view.frame.midY - control.bounds.midY) <= 1.0
        })
    }
    ...
    func testPagerAttachesOnlySelectedPageAndImmediateNeighbors() {
        let children = (0..<8).map { _ in UIViewController() }
        let controller = AetherPageViewController(
            pages: children.map { AetherPageViewController.Page(viewController: $0) },
            selectedIndex: 4,
            installViewPagerAsTopBarAccessory: false
        )
        ...
        XCTAssertEqual(
            Set(children.enumerated().compactMap { $0.element.parent === controller ? $0.offset : nil }),
            Set([3, 4, 5])
        )

        controller.setSelectedIndex(6, animated: false)
...
private extension UIView {
    ...
    func descendantViews(accessibilityLabel label: String) -> [UIView] {
        var result: [UIView] = []
        if accessibilityLabel == label {
            result.append(self)
        }
        for subview in subviews {
            result.append(contentsOf: subview.descendantViews(accessibilityLabel: label))
        }
        return result
    }
}
