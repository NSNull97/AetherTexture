...
7. Поле остаётся без фокуса. Клавиатура и сдвиг контента появляются
   только после явного тапа пользователя в `UITextField`.

### Жизненный цикл деактивации

1. Тап по close-кнопке (или вызов
   ``AetherSearchController/deactivate()``).
2. Если поле было сфокусировано, клавиатура скрывается одновременно с restore UI.
...
```swift
search.activate()    // раскрывает search UI без показа клавиатуры
...
При активации pill сжимается и появляется close-кнопка справа. Клавиатура
показывается и поднимает pill только после явного тапа в поле.
...
`activatesOnAppear = true`; по умолчанию это поведение выключено.
...
| `pillHeight` | `CGFloat` | `44.0` | Высота pill'а. |
| `horizontalInset` | `CGFloat` | `16.0` | Горизонтальный inset до pill'а. |
| `activatesOnAppear` | `Bool` | `false` | Opt-in автоматического first responder при добавлении. |
...
Search-слот внутри общей tab bar glass-поверхности, который отделяется в
круг только в minimized-состоянии (Apple Music style):
