...
Стек экранов с per-screen nav bar, style-aware переходами при push/pop,
...
- **Per-screen nav bar.** Каждый ``AetherViewController`` владеет собственным
  ...
  контроллером во время push/pop. Liquid Glass использует morph между двумя
  одновременно видимыми surfaces; Legacy сохраняет движение без glass-эффекта.
...
- **App appearance + local overrides.**
  ...
  Описание трёх стилей — в <doc:Appearance>.
...
| Метод | Назначение |
|---|---|
| `init(mode:)` | Создание с app-level appearance. |
| `init(rootViewController:mode:)` | Создание с первым экраном и app-level appearance. |
| `init(mode:theme:)` / `init(rootViewController:mode:theme:)` | Deprecated source-compatible adapters старой полной темы. |
...
| Метод | Назначение |
|---|---|
| ``AetherNavigationController/updateTheme(_:)`` | Deprecated adapter старой полной темы; app-level код использует `updateAppearance(_:)`. |
