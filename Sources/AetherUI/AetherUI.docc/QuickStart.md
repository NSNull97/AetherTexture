...
рекомендуется ознакомиться с <doc:Appearance>, <doc:ViewController>,
<doc:NavigationController> и <doc:TabBar> для углублённого изучения.

> Tip: Для интерактивного ознакомления откройте Xcode-проект `Example/Example.xcodeproj`
...
## Выбор appearance style

В declarative application bootstrap style задаётся один раз:
...
Также доступны `.liquidGlassV1` и `.liquidGlassV2`. Legacy — классическая
UIKit-тема на `UIBlurEffect.Style.systemChromeMaterial`, без `UIGlassEffect` и
Liquid Glass renderer; это не автоматический fallback для старой версии iOS.
Default остаётся `.liquidGlassV1`.

Для live-переключения существующей иерархии:

```swift
AetherApplicationRuntime.shared?.updateAppearanceStyle(.liquidGlassV2)
```

Подробнее о миграции `.iOS26/.iOS27`, Codable и platform fallback —
<doc:Appearance>.
...
- Собственный ``NavigationBarView`` со style-aware surface, edge-эффектом там,
  где он разрешён appearance, и back-кнопкой.
...
- При push выполняется style-aware переход между nav bar'ами: Liquid Glass
  сохраняет morph, Legacy использует обычную UIKit-анимацию поверхности.
