...
Per-screen navigation bar со style-aware surface, edge-эффектом, accessory-зонами,
...
обеспечивает синхронный style-aware переход между двумя барами при анимации.

Бар поддерживает:

- Стандартный `UINavigationItem` с `title`, `titleView`, `leftBarButtonItems`,
  `rightBarButtonItems`.
- Низкоуровневую тему ``NavigationBarTheme`` с двумя renderer modes:
  `.legacy` (UIKit-style) и `.glass` (Liquid Glass; native API на iOS 26+).
...
- Accessory-контент через ``AetherViewController/topBarAccessory`` —
  ...
  него (`.replacement`).
- Кнопки в едином surface-контейнере: Liquid Glass использует
  `GlassControlGroup` с морфом, Legacy — классические control states.
...
| `style` | ``NavigationBarStyle`` | `.glass` | Deprecated coarse layout switch: `.legacy` (UIKit) или `.glass` (Liquid Glass). |
| `appearanceStyle` | ``AetherAppearanceStyle`` | `.liquidGlassV1` | Канонический renderer generation. Явный `.legacy` переводит и coarse `style` в `.legacy`. |
...
| `defaultContentHeight` | `CGFloat` | `60.0` | Высота content-area (область кнопок и title); canonical app-level Legacy централизованно разрешает её в 44 pt. |

### Preset для liquid glass

Для типового Liquid Glass look используется factory:
...
Классический UIKit-style: фон на
`UIBlurEffect.Style.systemChromeMaterial`, separator-strip снизу и обычные
control states без `UIGlassEffect`. Высота content-area — 44 pt. App-level
`.legacy` выбирает этот renderer независимо от версии ОС.
...
Liquid Glass: прозрачный фон с системным `UIGlassEffect` на iOS 26+, кнопки
в едином `GlassControlGroup`-капсуле с автоматическим морфом. На более ранней
системе используется compatibility glass backend, а не app-level Legacy.

| `glassStyle` | Описание |
|---|---|
| `.default` | Panel-tinted glass (заметная подложка для читаемости поверх любого фона). |
| `.strong` | Усиленный Liquid Glass v2 material. |
...
открыть ``ContextMenuController`` при тапе:
...
системного `UIButton.menu` с `showsMenuAsPrimaryAction = true`. На iOS 15+
используется тот же быстрый trigger без legacy fallback.
