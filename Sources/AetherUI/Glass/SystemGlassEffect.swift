...
/// in dialog / toolbar / tooltip surfaces. Legacy always resolves to the
/// public `.systemChromeMaterial` blur. With a Liquid Glass appearance on
/// iOS 26+ this returns `UIGlassEffect`; older systems use the matching
/// public material fallback.
public enum SystemGlassEffectStyle: Equatable, Sendable {
    /// Dialog/card surface. Regular opacity.
    case regular
    /// Denser chrome surface used by the Liquid Glass v2 appearance preset.
    case strong
...
public enum SystemGlassEffect {
    ...
    public static func make(
        ...
        return make(
            style: AetherGlassConfig.current.style,
            isDark: isDark,
