...
open class VisualEffectView: UIVisualEffectView {
    ...
    private let blurEffect: UIBlurEffect = {
        ...
        #else
        guard let effectType = NSClassFromString(ObfuscatedSymbols.uiCustomBlurEffect) as? UIBlurEffect.Type else {
            return UIBlurEffect(style: .systemMaterial)
        }
        return effectType.init()
        #endif
    }()
    ...
    open var colorTint: UIColor? {
        get {
            #if APPSTORE_SAFE
            return customSnapshot.colorTint
            #else
    ...
    open var colorTintAlpha: CGFloat {
        get { return customSnapshot.colorTint?.cgColor.alpha ?? 0.0 }
    ...
    open var blurRadius: CGFloat {
        get {
            #if APPSTORE_SAFE
            return customSnapshot.blurRadius
            #else
    ...
    open var saturation: CGFloat {
        get {
            #if APPSTORE_SAFE
            return customSnapshot.saturation
            #else
            return _value(forKey: .saturation) ?? 1.0
            #endif
        }
        set {
    ...
    open var scale: CGFloat {
        get {
            #if APPSTORE_SAFE
            return customSnapshot.scale
            #else
            return _value(forKey: .samplingScale) ?? 1.0
            #endif
        }
        set {
...
private extension VisualEffectView {
    ...
    /// Creates the public iOS 26 glass effect.
