...
public final class EdgeEffectView: UIView, AetherAppearanceConsumer {
    public enum Edge {
        case top
        case bottom
    }
    ...
    private var lastUpdateRequest: UpdateRequest?
    ...
    public override init(frame: CGRect) {
        ...
        self.contentMaskView.contentMode = .scaleToFill
    ...
    public func update(
        ...
        let blur = blur && resolvedStyle.usesLiquidGlass
        ...
        let resolvedContent = content?.resolvedColor(with: traitCollection)
        if !transition.isAnimated {
            let signature = UpdateSignature(
                contentRGBA: Self.packRGBA(resolvedContent),
                blur: blur,
                alpha: alpha,
                size: rect.size,
                edge: edge,
        ...
        let useFill: Bool
        if let content = resolvedContent, content.cgColor.alpha > 0.001 {
            if transition.isAnimated {
        ...
        if blur {
            ...
            do {
                ...
                if solidBlur {
                    ...
                    let mask = blurMaskView ?? UIImageView()
                    mask.contentMode = .scaleToFill
                    blurMaskView = mask
                    effectView.mask = mask
            ...
            if solidBlur {
                ...
                if let current = blurView {
                    ...
                    visualEffectView.blurRadius = blurRadiusAtEdge
                    visualEffectView.colorTint = content
                    visualEffectView.colorTintAlpha = alpha
                ...
                // the solid edge (position 0, alpha = 1 → full `maxBlurRadius`)
                // to the transparent edge (position 1, alpha = `minFraction`
                // → `blurRadiusAtFade` after the radius scale). The constant
                // zone (the rest of the frame, behind the fade) inherits the
    ...
    func aetherApplyAppearance(_ appearance: AetherAppearance, animated: Bool) {
        guard let request = lastUpdateRequest else {
            if !(appearanceStyleOverride ?? appearance.style).usesLiquidGlass {
                tearDownBlurRenderers()
            }
            return
        }
        lastUpdateSignature = nil
        update(
            ...
            blurRadiusAtEdge: request.blurRadiusAtEdge,
            blurRadiusAtFade: request.blurRadiusAtFade,
            solidBlur: request.solidBlur,
            fadeCurveExponent: request.fadeCurveExponent,
            minimumFadeAlpha: request.minimumFadeAlpha,
            prefersStaticVariableBlurMask: request.prefersStaticVariableBlurMask,
            transition: .immediate
        )
    }

    private func tearDownBlurRenderers() {
        blurView?.effect = nil
...
final class BackdropBlurHostView: UIView {
    ...
    private static func createBackdropLayer() -> CALayer? {
        ...
        guard let allocated = cls.perform(allocSel)?.takeUnretainedValue() as AnyObject?,
              let initialized = allocated.perform(initSel)
        else { return nil }
        // The result still carries the +1 from `alloc/init`. Hand it to ARC;
        // `takeUnretainedValue()` here permanently leaked each backdrop layer.
        return initialized.takeRetainedValue() as? CALayer
    }
}
#else
/// Type-compatible placeholder used only so EdgeEffectView's stored
/// properties keep the same layout in safe builds. The safe update branch
/// renders a masked UIVisualEffectView and never instantiates this class.
final class BackdropBlurHostView: UIView {
...
public final class VariableBlurEffect {
    ...
    public init(
        ...
        if #available(iOS 26.0, *), !usesStaticMaskImage {
            ...
            imageMaskView.contentMode = .scaleToFill
            imageMaskView.layer.name = "mask_source"
            ...
            self.layer.addSublayer(imageMaskView.layer)
        } else {
    ...
    deinit {
        imageMaskView?.layer.removeFromSuperlayer()
    }
    ...
    public func update(
        ...
        if isGradientUpdated {
            if let inwardsExtension = params.placement.inwardsExtension {
                ...
                // Full-size mask, no resizableImage stretching. Drawing a
                // 1×fadeHeight image and then stretching it through capInsets
                // bled the constant-zone alpha across the entire frame, so a
                // ramp like 10pt → 0pt rendered as a sheet of full-strength
                // blur. Instead we draw the mask at the actual frame size:
                // the constant zone (alpha = `alphas[0]`, the value at the
                // solid edge) sits flush with the solid side, then the fade
                // zone ramps from `alphas[0]` to `alphas[last]` over exactly
                // `gradient.height` points toward the transparent side.
                let fadeHeight = max(0.0, params.gradient.height)
                let totalHeight = max(1.0, params.size.height)
                let constantHeight = max(0.0, totalHeight - fadeHeight)
                let alphas = params.gradient.alpha
                let positions = params.gradient.positions
                let isBottom = params.placement.position == .bottom

                self.gradientImage = generateImage(CGSize(width: 1.0, height: totalHeight), rotatedContext: { size, context in
                    context.clear(CGRect(origin: .zero, size: size))

                    let constantRect: CGRect
                    let gradientStart: CGPoint
                    let gradientEnd: CGPoint
                    if isBottom {
                        // Solid edge at the bottom of the image. Constant zone
                        // hugs the bottom; gradient ramps upward toward the
                        // transparent side.
                        constantRect = CGRect(x: 0.0, y: size.height - constantHeight, width: size.width, height: constantHeight)
                        gradientStart = CGPoint(x: 0.0, y: size.height - constantHeight)
                        gradientEnd = CGPoint(x: 0.0, y: max(0.0, size.height - constantHeight - fadeHeight))
                    } else {
                        // Solid edge at the top of the image.
                        constantRect = CGRect(x: 0.0, y: 0.0, width: size.width, height: constantHeight)
                        gradientStart = CGPoint(x: 0.0, y: constantHeight)
                        gradientEnd = CGPoint(x: 0.0, y: min(size.height, constantHeight + fadeHeight))
                    }

                    if constantHeight > 0.0, let firstAlpha = alphas.first {
                        context.setFillColor(UIColor(white: 1.0, alpha: firstAlpha).cgColor)
                        context.fill(constantRect)
                    }
                    if fadeHeight > 0.0, alphas.count >= 2 {
                        let colorSpace = CGColorSpaceCreateDeviceRGB()
                        let colors = alphas.map { UIColor(white: 1.0, alpha: $0).cgColor }
                        var locs = positions
        ...
        if let imageMaskView {
            if isGradientUpdated {
                imageMaskView.image = self.gradientImage
            }
            transition.setFrame(layer: self.layer, frame: CGRect(origin: .zero, size: size))
            transition.setFrame(view: imageMaskView, frame: CGRect(origin: .zero, size: size))
        } else {
            transition.setFrame(layer: self.layer, frame: CGRect(origin: .zero, size: size))
...
public final class VariableBlurView: UIView {
    ...
    #if APPSTORE_SAFE
    private let safeEffectView: UIVisualEffectView?
    private let safeMaskView: UIView?
    #endif
    ...
    public init(maxBlurRadius: CGFloat = 20.0, usesStaticMaskImage: Bool = false) {
        ...
        let safeEffectView = UIVisualEffectView(effect: UIBlurEffect(style: .systemMaterial))
        safeEffectView.isUserInteractionEnabled = false
        self.safeEffectView = safeEffectView
        let safeMaskView = UIView()
        safeMaskView.layer.contentsGravity = .resize
        self.safeMaskView = safeMaskView
        safeEffectView.mask = safeMaskView
        #else
    ...
    deinit {
        effect = nil
        mainEffectLayer?.filters = nil
        mainEffectLayer?.removeFromSuperlayer()
        mainEffectLayer = nil
    }
    ...
    public override func layoutSubviews() {
        ...
        #if APPSTORE_SAFE
        safeEffectView?.frame = bounds
        safeMaskView?.frame = bounds
        #endif
    }
    ...
    public func update(
        ...
        #if APPSTORE_SAFE
        safeEffectView?.frame = CGRect(origin: .zero, size: size)
        safeMaskView?.frame = CGRect(origin: .zero, size: size)
        safeMaskView?.layer.contents = EdgeEffectView.generateTaperedGradient(
            totalHeight: size.height,
            fadeHeight: min(size.height, gradient.height),
            edge: isInverted ? .bottom : .top,
            curveExponent: 1.0,
            minimumAlpha: gradient.alpha.last ?? 0.0
        )?.cgImage
        #else
        effect?.update(
            size: size,
