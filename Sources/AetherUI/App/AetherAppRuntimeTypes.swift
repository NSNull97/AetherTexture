...
extension AetherScreenNode: AetherWindowContent {
    public var aetherWindowContentController: UIViewController {
        AetherScreenController(screenNode: self)
    }
}
...
public struct AetherAppEnvironmentValues {
    ...
    /// Source-compatible shorthand for the style carried by `appearance`.
    ///
    /// `appearance` is the single source of truth. Writing a style installs
    /// that style's canonical preset, matching `updateAppearanceStyle(_:)`.
    public var appearanceStyle: AetherAppearanceStyle {
        get { appearance.style }
        set { appearance = AetherAppearance(style: newValue) }
    }
