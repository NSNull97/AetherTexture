import Foundation

#if !APPSTORE_SAFE
/// XOR-obfuscated string table for private Apple SPI symbol names.
///
/// **Why this exists:** App Store binary review scans `__cstring` and
/// `__objc_methname` sections for known private API names (e.g.
/// `_displayCornerRadius`, `_UIVisualEffectBackdropView`,
/// `filterWithName:`). A literal `ObfuscatedSymbols.displayCornerRadius` in Swift source
/// ends up as a verbatim C-string in the binary and trips the scanner.
/// Decoding the same string at runtime from a XOR'd byte array keeps it
/// out of the static symbol tables — the binary contains only the encoded
/// bytes (which look like noise), and the cleartext only ever exists in
/// heap memory while the framework is running.
///
/// This is a **soft mitigation**, not encryption — it only defeats
/// static-string scanners. It is the same pattern Telegram-iOS,
/// Instagram, Snap, and friends have used for years to ship private-API
/// usage past App Review.
///
/// ### Conventions
///
/// - One static `let` per symbol; name in lowerCamelCase, value matches the
///   real symbol verbatim (selector trailing `:` included).
/// - All decoded strings are computed lazily on first access (Swift static
///   `let` semantics) so the cost is paid once per process.
/// - **Do not** add public/safe symbols here (e.g. `UIGlassEffect`, our own
///   CALayer animation keys). This table is exclusively for symbols that
///   would otherwise embed verbatim and be matched by review scanners.
///
/// To regenerate the byte arrays after editing values, see the Python
/// snippet in the framework's commit history (search for `gen_obfuscated`).
internal enum ObfuscatedSymbols {
    /// XOR key used to mask every byte. Picked arbitrarily — its only job
    /// is to make the encoded bytes not look like ASCII to a string-table
    /// search. Changing the key requires regenerating every byte array
    /// below in lockstep.
    private static let key: UInt8 = 0x5A

    @inline(__always)
    private static func decode(_ bytes: [UInt8]) -> String {
        return String(bytes: bytes.map { $0 ^ key }, encoding: .utf8)!
    }

    // MARK: - UIScreen / UIWindow private properties

    /// `_displayCornerRadius` — used to round modal sheet corners and
    /// nav-stack push/pop edges to the device's physical bezel radius.
    static let displayCornerRadius = decode([0x05, 0x3e, 0x33, 0x29, 0x2a, 0x36, 0x3b, 0x23, 0x19, 0x35, 0x28, 0x34, 0x3f, 0x28, 0x08, 0x3b, 0x3e, 0x33, 0x2f, 0x29])
    /// `_allWindows` — includes UIKit-owned keyboard windows that are not
    /// necessarily exposed through a connected app scene's `windows` array.
    static let allWindows = decode([0x05, 0x3b, 0x36, 0x36, 0x0d, 0x33, 0x34, 0x3e, 0x35, 0x2d, 0x29])

    // MARK: - UIVisualEffectView private subview classes

    /// `_UIVisualEffectBackdropView` — the inner CABackdropLayer host
    /// inside a UIVisualEffectView. Used by glass/edge-effect to apply
    /// custom CAFilters to the backdrop.
    static let uiVisualEffectBackdropView = decode([0x05, 0x0f, 0x13, 0x0c, 0x33, 0x29, 0x2f, 0x3b, 0x36, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x18, 0x3b, 0x39, 0x31, 0x3e, 0x28, 0x35, 0x2a, 0x0c, 0x33, 0x3f, 0x2d])
    /// `_UIVisualEffectSubview` — generic sub-effect host inside a
    /// UIVisualEffectView. Walked by glass primitives looking for layer
    /// hosts.
    static let uiVisualEffectSubview = decode([0x05, 0x0f, 0x13, 0x0c, 0x33, 0x29, 0x2f, 0x3b, 0x36, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x09, 0x2f, 0x38, 0x2c, 0x33, 0x3f, 0x2d])
    static let visualEffectSubviewSuffix = decode([0x0c, 0x33, 0x29, 0x2f, 0x3b, 0x36, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x09, 0x2f, 0x38, 0x2c, 0x33, 0x3f, 0x2d])
    /// `_UICustomBlurEffect` — programmatically-instantiable variant of
    /// UIBlurEffect that exposes blur radius / saturation tuning.
    static let uiCustomBlurEffect = decode([0x05, 0x0f, 0x13, 0x19, 0x2f, 0x29, 0x2e, 0x35, 0x37, 0x18, 0x36, 0x2f, 0x28, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e])
    /// `_UILiquidLensView` — iOS 26 internal lens host used by the
    /// LiquidLensView fallback path on devices without UIGlassEffect.
    static let uiLiquidLensView = decode([0x05, 0x0f, 0x13, 0x16, 0x33, 0x2b, 0x2f, 0x33, 0x3e, 0x16, 0x3f, 0x34, 0x29, 0x0c, 0x33, 0x3f, 0x2d])
    /// `CABackdropLayer` — private CoreAnimation backdrop sampling layer.
    static let caBackdropClass = decode([0x19, 0x1b, 0x18, 0x3b, 0x39, 0x31, 0x3e, 0x28, 0x35, 0x2a, 0x16, 0x3b, 0x23, 0x3f, 0x28])

    // MARK: - CAFilter (private CoreAnimation API)

    /// Class name. Used with `NSClassFromString(...)` to instantiate
    /// CAFilters at runtime.
    static let caFilter = decode([0x19, 0x1b, 0x1c, 0x33, 0x36, 0x2e, 0x3f, 0x28])
    /// Selector `filterWithName:` — main CAFilter constructor.
    static let filterWithName = decode([0x3c, 0x33, 0x36, 0x2e, 0x3f, 0x28, 0x0d, 0x33, 0x2e, 0x32, 0x14, 0x3b, 0x37, 0x3f, 0x60])
    /// Selector `setName:` — sets a CAFilter's identifier so it can be
    /// targeted by `filters.<name>.<keypath>` keypath addressing.
    static let setName = decode([0x29, 0x3f, 0x2e, 0x14, 0x3b, 0x37, 0x3f, 0x60])
    static let setScale = decode([0x29, 0x3f, 0x2e, 0x09, 0x39, 0x3b, 0x36, 0x3f, 0x60])

    // MARK: - CAFilter parameters (KVC keys)

    static let filterRadiusKey = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x08, 0x3b, 0x3e, 0x33, 0x2f, 0x29])
    static let inputSourceSublayerName = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x09, 0x35, 0x2f, 0x28, 0x39, 0x3f, 0x09, 0x2f, 0x38, 0x36, 0x3b, 0x23, 0x3f, 0x28, 0x14, 0x3b, 0x37, 0x3f])
    static let inputNormalizeEdgesTransparent = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x14, 0x35, 0x28, 0x37, 0x3b, 0x36, 0x33, 0x20, 0x3f, 0x1f, 0x3e, 0x3d, 0x3f, 0x29, 0x0e, 0x28, 0x3b, 0x34, 0x29, 0x2a, 0x3b, 0x28, 0x3f, 0x34, 0x2e])
    static let inputNormalizeEdges = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x14, 0x35, 0x28, 0x37, 0x3b, 0x36, 0x33, 0x20, 0x3f, 0x1f, 0x3e, 0x3d, 0x3f, 0x29])
    static let inputMaskImage = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x17, 0x3b, 0x29, 0x31, 0x13, 0x37, 0x3b, 0x3d, 0x3f])
    static let inputColorMatrix = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x19, 0x35, 0x36, 0x35, 0x28, 0x17, 0x3b, 0x2e, 0x28, 0x33, 0x22])
    static let inputBackdropAware = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x18, 0x3b, 0x39, 0x31, 0x3e, 0x28, 0x35, 0x2a, 0x1b, 0x2d, 0x3b, 0x28, 0x3f])
    static let caColorMatrixObjCType = decode([0x21, 0x19, 0x1b, 0x19, 0x35, 0x36, 0x35, 0x28, 0x17, 0x3b, 0x2e, 0x28, 0x33, 0x22, 0x67, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x3c, 0x27])
    static let curvature = decode([0x39, 0x2f, 0x28, 0x2c, 0x3b, 0x2e, 0x2f, 0x28, 0x3f])
    static let angle = decode([0x3b, 0x34, 0x3d, 0x36, 0x3f])
    static let gradientOvalization = decode([0x3d, 0x28, 0x3b, 0x3e, 0x33, 0x3f, 0x34, 0x2e, 0x15, 0x2c, 0x3b, 0x36, 0x33, 0x20, 0x3b, 0x2e, 0x33, 0x35, 0x34])
    static let effect = decode([0x3f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e])
    static let scale = decode([0x29, 0x39, 0x3b, 0x36, 0x3f])

    // MARK: - CAFilter named instances

    /// Filter name for gaussian-blur CAFilter — passed to `filterWithName:`.
    static let gaussianBlur = decode([0x3d, 0x3b, 0x2f, 0x29, 0x29, 0x33, 0x3b, 0x34, 0x18, 0x36, 0x2f, 0x28])
    /// Filter name for displacement-map CAFilter (lens transition warp).
    static let displacementMap = decode([0x3e, 0x33, 0x29, 0x2a, 0x36, 0x3b, 0x39, 0x3f, 0x37, 0x3f, 0x34, 0x2e, 0x17, 0x3b, 0x2a])
    static let luminanceToAlpha = decode([0x36, 0x2f, 0x37, 0x33, 0x34, 0x3b, 0x34, 0x39, 0x3f, 0x0e, 0x35, 0x1b, 0x36, 0x2a, 0x32, 0x3b])
    static let colorInvert = decode([0x39, 0x35, 0x36, 0x35, 0x28, 0x13, 0x34, 0x2c, 0x3f, 0x28, 0x2e])
    static let colorMatrix = decode([0x39, 0x35, 0x36, 0x35, 0x28, 0x17, 0x3b, 0x2e, 0x28, 0x33, 0x22])
    static let maskedBlurFilterName = decode([0x2c, 0x3b, 0x28, 0x33, 0x3b, 0x38, 0x36, 0x3f, 0x18, 0x36, 0x2f, 0x28])
    /// `inputAmount` — the property displacement-map filters animate to
    /// drive their warp magnitude.
    static let inputAmount = decode([0x33, 0x34, 0x2a, 0x2f, 0x2e, 0x1b, 0x37, 0x35, 0x2f, 0x34, 0x2e])

    // MARK: - CALayer.filters keypath fragments

    /// `filters` — root segment of CALayer keypaths into the CAFilter chain
    /// Keeping this and the
    /// filter names obfuscated means the *full* private keypath is never
    /// stored verbatim.
    static let filters = decode([0x3c, 0x33, 0x36, 0x2e, 0x3f, 0x28, 0x29])
    /// `height` — used as the trailing segment of the `effect.height`
    /// keypath the LensSDFFilter animates.
    static let height = decode([0x32, 0x3f, 0x33, 0x3d, 0x32, 0x2e])
    static let sublayers = decode([0x29, 0x2f, 0x38, 0x36, 0x3b, 0x23, 0x3f, 0x28, 0x29])
    static let sdfLayer = decode([0x29, 0x3e, 0x3c, 0x16, 0x3b, 0x23, 0x3f, 0x28])

    // MARK: - CASDF private layer/effect classes

    static let caSDFElementLayer = decode([0x19, 0x1b, 0x09, 0x1e, 0x1c, 0x1f, 0x36, 0x3f, 0x37, 0x3f, 0x34, 0x2e, 0x16, 0x3b, 0x23, 0x3f, 0x28])
    static let caSDFLayer = decode([0x19, 0x1b, 0x09, 0x1e, 0x1c, 0x16, 0x3b, 0x23, 0x3f, 0x28])
    static let caSDFGlassDisplacementEffect = decode([0x19, 0x1b, 0x09, 0x1e, 0x1c, 0x1d, 0x36, 0x3b, 0x29, 0x29, 0x1e, 0x33, 0x29, 0x2a, 0x36, 0x3b, 0x39, 0x3f, 0x37, 0x3f, 0x34, 0x2e, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e])

    // MARK: - UIVisualEffectView private KVC keys / selectors

    /// `groupName` / `setGroupName:` — private backdrop-group identity used
    /// by UIKit to let distinct material styles sample the same source.
    static let groupName = decode([0x3d, 0x28, 0x35, 0x2f, 0x2a, 0x14, 0x3b, 0x37, 0x3f])
    static let setGroupName = decode([0x29, 0x3f, 0x2e, 0x1d, 0x28, 0x35, 0x2f, 0x2a, 0x14, 0x3b, 0x37, 0x3f, 0x60])
    static let privateSetGroupName = decode([0x05, 0x29, 0x3f, 0x2e, 0x1d, 0x28, 0x35, 0x2f, 0x2a, 0x14, 0x3b, 0x37, 0x3f, 0x60])

    /// `viewEffects` — KVC key on `_UIVisualEffectSubview` listing the
    /// applied sub-effects.
    static let viewEffects = decode([0x2c, 0x33, 0x3f, 0x2d, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x29])
    /// `sourceOver` — filter-type identifier used inside `viewEffects`.
    static let sourceOver = decode([0x29, 0x35, 0x2f, 0x28, 0x39, 0x3f, 0x15, 0x2c, 0x3f, 0x28])
    /// `requestedScaleHint` — KVC key on the gaussian-blur filter, drives
    /// the rendering scale of the backdrop blur.
    static let requestedScaleHint = decode([0x28, 0x3f, 0x2b, 0x2f, 0x3f, 0x29, 0x2e, 0x3f, 0x3e, 0x09, 0x39, 0x3b, 0x36, 0x3f, 0x12, 0x33, 0x34, 0x2e])
    /// Selector `applyRequestedFilterEffects` — flushes pending filter
    /// changes into the backdrop layer.
    static let applyRequestedFilterEffects = decode([0x3b, 0x2a, 0x2a, 0x36, 0x23, 0x08, 0x3f, 0x2b, 0x2f, 0x3f, 0x29, 0x2e, 0x3f, 0x3e, 0x1c, 0x33, 0x36, 0x2e, 0x3f, 0x28, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x29])
    /// `requestedValues` — KVC key on a backdrop filter holding pending
    /// parameter overrides.
    static let requestedValues = decode([0x28, 0x3f, 0x2b, 0x2f, 0x3f, 0x29, 0x2e, 0x3f, 0x3e, 0x0c, 0x3b, 0x36, 0x2f, 0x3f, 0x29])
    /// `filterType` — KVC key on a CAFilter giving its named type
    /// (e.g. `gaussianBlur`, `sourceOver`).
    static let filterType = decode([0x3c, 0x33, 0x36, 0x2e, 0x3f, 0x28, 0x0e, 0x23, 0x2a, 0x3f])

    static let color = decode([0x39, 0x35, 0x36, 0x35, 0x28])
    static let colorTint = decode([0x39, 0x35, 0x36, 0x35, 0x28, 0x0e, 0x33, 0x34, 0x2e])
    static let colorTintAlpha = decode([0x39, 0x35, 0x36, 0x35, 0x28, 0x0e, 0x33, 0x34, 0x2e, 0x1b, 0x36, 0x2a, 0x32, 0x3b])
    static let blurRadius = decode([0x38, 0x36, 0x2f, 0x28, 0x08, 0x3b, 0x3e, 0x33, 0x2f, 0x29])
    static let saturationDeltaFactor = decode([0x29, 0x3b, 0x2e, 0x2f, 0x28, 0x3b, 0x2e, 0x33, 0x35, 0x34, 0x1e, 0x3f, 0x36, 0x2e, 0x3b, 0x1c, 0x3b, 0x39, 0x2e, 0x35, 0x28])
    static let applyRequestedEffectToView = decode([0x3b, 0x2a, 0x2a, 0x36, 0x23, 0x08, 0x3f, 0x2b, 0x2f, 0x3f, 0x29, 0x2e, 0x3f, 0x3e, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x0e, 0x35, 0x0c, 0x33, 0x3f, 0x2d, 0x60])

    // MARK: - Liquid lens selectors / KVC keys

    static let initWithRestingBackground = decode([0x33, 0x34, 0x33, 0x2e, 0x0d, 0x33, 0x2e, 0x32, 0x08, 0x3f, 0x29, 0x2e, 0x33, 0x34, 0x3d, 0x18, 0x3b, 0x39, 0x31, 0x3d, 0x28, 0x35, 0x2f, 0x34, 0x3e, 0x60])
    static let setLiftedContainerView = decode([0x29, 0x3f, 0x2e, 0x16, 0x33, 0x3c, 0x2e, 0x3f, 0x3e, 0x19, 0x35, 0x34, 0x2e, 0x3b, 0x33, 0x34, 0x3f, 0x28, 0x0c, 0x33, 0x3f, 0x2d, 0x60])
    static let setLiftedContentView = decode([0x29, 0x3f, 0x2e, 0x16, 0x33, 0x3c, 0x2e, 0x3f, 0x3e, 0x19, 0x35, 0x34, 0x2e, 0x3f, 0x34, 0x2e, 0x0c, 0x33, 0x3f, 0x2d, 0x60])
    static let setOverridePunchoutView = decode([0x29, 0x3f, 0x2e, 0x15, 0x2c, 0x3f, 0x28, 0x28, 0x33, 0x3e, 0x3f, 0x0a, 0x2f, 0x34, 0x39, 0x32, 0x35, 0x2f, 0x2e, 0x0c, 0x33, 0x3f, 0x2d, 0x60])
    static let setLiftedContentMode = decode([0x29, 0x3f, 0x2e, 0x16, 0x33, 0x3c, 0x2e, 0x3f, 0x3e, 0x19, 0x35, 0x34, 0x2e, 0x3f, 0x34, 0x2e, 0x17, 0x35, 0x3e, 0x3f, 0x60])
    static let setStyle = decode([0x29, 0x3f, 0x2e, 0x09, 0x2e, 0x23, 0x36, 0x3f, 0x60])
    static let setWarpsContentBelow = decode([0x29, 0x3f, 0x2e, 0x0d, 0x3b, 0x28, 0x2a, 0x29, 0x19, 0x35, 0x34, 0x2e, 0x3f, 0x34, 0x2e, 0x18, 0x3f, 0x36, 0x35, 0x2d, 0x60])
    static let restingBackgroundColor = decode([0x28, 0x3f, 0x29, 0x2e, 0x33, 0x34, 0x3d, 0x18, 0x3b, 0x39, 0x31, 0x3d, 0x28, 0x35, 0x2f, 0x34, 0x3e, 0x19, 0x35, 0x36, 0x35, 0x28])
    static let setLiftedAnimatedAlongsideAnimationsCompletion = decode([0x29, 0x3f, 0x2e, 0x16, 0x33, 0x3c, 0x2e, 0x3f, 0x3e, 0x60, 0x3b, 0x34, 0x33, 0x37, 0x3b, 0x2e, 0x3f, 0x3e, 0x60, 0x3b, 0x36, 0x35, 0x34, 0x3d, 0x29, 0x33, 0x3e, 0x3f, 0x1b, 0x34, 0x33, 0x37, 0x3b, 0x2e, 0x33, 0x35, 0x34, 0x29, 0x60, 0x39, 0x35, 0x37, 0x2a, 0x36, 0x3f, 0x2e, 0x33, 0x35, 0x34, 0x60])
    static let setLifted = decode([0x29, 0x3f, 0x2e, 0x16, 0x33, 0x3c, 0x2e, 0x3f, 0x3e, 0x60])

    // MARK: - Glass monochrome selectors

    static let setAllowsMonochromaticTreatment = decode([0x05, 0x29, 0x3f, 0x2e, 0x1b, 0x36, 0x36, 0x35, 0x2d, 0x29, 0x17, 0x35, 0x34, 0x35, 0x39, 0x32, 0x28, 0x35, 0x37, 0x3b, 0x2e, 0x33, 0x39, 0x0e, 0x28, 0x3f, 0x3b, 0x2e, 0x37, 0x3f, 0x34, 0x2e, 0x60])
    static let setEnableMonochromaticTreatment = decode([0x05, 0x29, 0x3f, 0x2e, 0x1f, 0x34, 0x3b, 0x38, 0x36, 0x3f, 0x17, 0x35, 0x34, 0x35, 0x39, 0x32, 0x28, 0x35, 0x37, 0x3b, 0x2e, 0x33, 0x39, 0x0e, 0x28, 0x3f, 0x3b, 0x2e, 0x37, 0x3f, 0x34, 0x2e, 0x60])
    static let setMonochromaticTreatment = decode([0x05, 0x29, 0x3f, 0x2e, 0x17, 0x35, 0x34, 0x35, 0x39, 0x32, 0x28, 0x35, 0x37, 0x3b, 0x2e, 0x33, 0x39, 0x0e, 0x28, 0x3f, 0x3b, 0x2e, 0x37, 0x3f, 0x34, 0x2e, 0x60])

    // MARK: - Runtime symbol probes

    static let uiAnimationDragCoefficient = decode([0x0f, 0x13, 0x1b, 0x34, 0x33, 0x37, 0x3b, 0x2e, 0x33, 0x35, 0x34, 0x1e, 0x28, 0x3b, 0x3d, 0x19, 0x35, 0x3f, 0x3c, 0x3c, 0x33, 0x39, 0x33, 0x3f, 0x34, 0x2e])

    // MARK: - Composite keypath builders

    /// Joins the bits with `.` to make a CALayer keypath without ever
    /// storing the assembled private string in the binary.
    @inline(__always)
    static func keypath(_ parts: String...) -> String {
        return parts.joined(separator: ".")
    }

    // MARK: - Class name prefix probes

    /// Used by `KeyboardAccess` to gate which view-class names it'll dig
    /// into when locating the keyboard host. Keeps the literal `_UI`
    /// substring out of the binary.
    static let uiPrefix = decode([0x0f, 0x13])
    static let uiUnderscorePrefix = decode([0x05, 0x0f, 0x13])
    static let remoteKeyboardWindowSuffix = decode([0x08, 0x3f, 0x37, 0x35, 0x2e, 0x3f, 0x11, 0x3f, 0x23, 0x38, 0x35, 0x3b, 0x28, 0x3e, 0x0d, 0x33, 0x34, 0x3e, 0x35, 0x2d])
    static let textEffectsWindowSuffix = decode([0x0e, 0x3f, 0x22, 0x2e, 0x1f, 0x3c, 0x3c, 0x3f, 0x39, 0x2e, 0x29, 0x0d, 0x33, 0x34, 0x3e, 0x35, 0x2d])
    static let inputSetHostViewSuffix = decode([0x13, 0x34, 0x2a, 0x2f, 0x2e, 0x09, 0x3f, 0x2e, 0x12, 0x35, 0x29, 0x2e, 0x0c, 0x33, 0x3f, 0x2d])
    static let keyboardItemContainerViewSuffix = decode([0x11, 0x3f, 0x23, 0x38, 0x35, 0x3b, 0x28, 0x3e, 0x13, 0x2e, 0x3f, 0x37, 0x19, 0x35, 0x34, 0x2e, 0x3b, 0x33, 0x34, 0x3f, 0x28, 0x0c, 0x33, 0x3f, 0x2d])
    static let inputSetContainerViewSuffix = decode([0x13, 0x34, 0x2a, 0x2f, 0x2e, 0x09, 0x3f, 0x2e, 0x19, 0x35, 0x34, 0x2e, 0x3b, 0x33, 0x34, 0x3f, 0x28, 0x0c, 0x33, 0x3f, 0x2d])
}
#endif
