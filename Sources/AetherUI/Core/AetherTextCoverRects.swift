    ...
    private static func normalizedAttributedText(for label: UILabel) -> NSAttributedString {
        ...
        if let attributedText = label.attributedText, attributedText.length > 0 {
            ...
            source = NSAttributedString(
                string: label.text ?? "",
                attributes: [
                    .font: label.font ?? UIFont.aetherScaledSystemFont(ofSize: UIFont.systemFontSize),
        ...
        mutable.enumerateAttribute(.font, in: fullRange, options: []) { value, range, _ in
            if value == nil {
                mutable.addAttribute(.font, value: label.font ?? UIFont.aetherScaledSystemFont(ofSize: UIFont.systemFontSize), range: range)
            }
        }
