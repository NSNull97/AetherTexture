...
private final class AetherEmitterView: UIView {
    ...
    private func updateEmitterRects() {
        ...
        #if APPSTORE_SAFE
        // CAEmitterLayer's public API supports one rectangular emitter. The
        // union keeps the public fallback spatially close to the private
        // multi-rect implementation while the cover mask clips any overflow.
        let emitterRect = rects.reduce(CGRect.null) { $0.union($1) }
        emitterLayer.emitterShape = .rectangle
        emitterLayer.emitterPosition = CGPoint(x: emitterRect.midX, y: emitterRect.midY)
        emitterLayer.emitterSize = emitterRect.size
        #else
    ...
    private func enableFingerAttractor(at point: CGPoint) {
        #if APPSTORE_SAFE
        // Public approximation: move the rectangular emitter to the touch.
        // The existing birth-rate/velocity animation still provides the
        // reveal burst without CAEmitterBehavior.
...
private var aetherStoredCoverViewKey: UInt8 = 0
private var aetherStoredInvisibleInkKey: UInt8 = 0
private var aetherStoredInvisibleInkOwnerAlphaKey: UInt8 = 0

private extension UIView {
    var aetherStoredCoverView: AetherParticleContentCoverView? {
        get { objc_getAssociatedObject(self, &aetherStoredCoverViewKey) as? AetherParticleContentCoverView }
        set { objc_setAssociatedObject(self, &aetherStoredCoverViewKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }

    var aetherStoredInvisibleInk: AetherInvisibleInkView? {
        get { objc_getAssociatedObject(self, &aetherStoredInvisibleInkKey) as? AetherInvisibleInkView }
        set { objc_setAssociatedObject(self, &aetherStoredInvisibleInkKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }

    var aetherStoredInvisibleInkOwnerAlpha: NSNumber? {
        get { objc_getAssociatedObject(self, &aetherStoredInvisibleInkOwnerAlphaKey) as? NSNumber }
        set { objc_setAssociatedObject(self, &aetherStoredInvisibleInkOwnerAlphaKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
...
public extension UIView {
    ...
    func setInvisibleInk(
        ...
        if self is UILabel, let hostView = superview {
...
public extension TextNode {
    ...
    func setInvisibleInk(
        ...
        let inkView: AetherInvisibleInkView
        if let currentInkView = ownerView.aetherStoredInvisibleInk {
            inkView = currentInkView
            inkView.configuration = configuration
        } else {
            inkView = AetherInvisibleInkView(configuration: configuration)
            inkView.ownerView = ownerView
            ownerView.aetherStoredInvisibleInk = inkView
            ownerView.addSubview(inkView)
        }

        let coverRects = textCoverRects()
        let textColor: UIColor
        if let attributedText, attributedText.length > 0 {
            textColor = attributedText.attribute(.foregroundColor, at: 0, effectiveRange: nil) as? UIColor ?? ownerView.tintColor
        } else {
            textColor = ownerView.tintColor
        }

        inkView.isHidden = false
        inkView.update(
            size: ownerView.bounds.size,
            color: configuration.particleColor,
            textColor: textColor,
            rects: coverRects.lineRects,
            wordRects: coverRects.wordRects
        ...
        if let hostView = ownerView.superview {
            if inkView.superview !== hostView {
                inkView.removeFromSuperview()
                hostView.addSubview(inkView)
            }
            inkView.frame = ownerView.convert(ownerView.bounds, to: hostView)
            inkView.autoresizingMask = []
            hostView.bringSubviewToFront(inkView)
            ownerView.aetherStoreInvisibleInkOwnerAlphaIfNeeded()
        } else {
            if inkView.superview !== ownerView {
                inkView.removeFromSuperview()
                ownerView.addSubview(inkView)
            }
            inkView.frame = ownerView.bounds
            inkView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            ownerView.bringSubviewToFront(inkView)
        }
    ...
    func removeInk(animated: Bool = false) {
        view.removeInk(animated: animated)
    }
}
...
private final class AetherRevealMaskController {
    ...
    private func animate(
        ...
        spotNode.image = maskImage
        spotNode.contentMode = .scaleToFill
        spotNode.view.alpha = fadesSpotIn ? 0.0 : 1.0
        spotNode.view.bounds = CGRect(origin: .zero, size: CGSize(width: size.width * 3.0, height: size.height * 3.0))
        spotNode.view.layer.anchorPoint = CGPoint(
            x: size.width > 0.0 ? clippedPoint.x / size.width : 0.5,
            y: size.height > 0.0 ? clippedPoint.y / size.height : 0.5
        )
        spotNode.view.layer.position = clippedPoint
        spotNode.view.transform = CGAffineTransform(scaleX: 0.3333, y: 0.3333)
        ...
        maskView.addSubview(spotNode.view)
        if includesFillFade {
        ...
        UIView.animate(
            ...
            animations: {
                self.spotNode.view.transform = CGAffineTransform(scaleX: 10.5 + scaleAddition, y: 10.5 + scaleAddition)
            },
            completion: { _ in
                targetView.mask = nil
                completion()
