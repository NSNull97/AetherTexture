...
public final class TabSelectionRecognizer: UIGestureRecognizer {
    ...
    private var previousVelocitySample: (location: CGPoint, timestamp: TimeInterval)?
    private var currentVelocitySample: (location: CGPoint, timestamp: TimeInterval)?
    ...
    override public func reset() {
        ...
        initialLocation = nil
        currentLocation = nil
        previousVelocitySample = nil
        currentVelocitySample = nil
    }
    ...
    override public func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if let touch = touches.first, let location = initialLocation {
            currentVelocitySample = (location, touch.timestamp)
        }
    ...
    override public func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if let touch = touches.first {
            ...
            recordVelocitySample(location: location, timestamp: touch.timestamp)
        }
    ...
    public func velocity(in view: UIView?) -> CGPoint {
        guard let previousVelocitySample,
              let currentVelocitySample else {
            return .zero
        }
        let duration = currentVelocitySample.timestamp - previousVelocitySample.timestamp
        guard duration > 1.0 / 1_000.0 else { return .zero }

        let velocity = CGPoint(
            x: (currentVelocitySample.location.x - previousVelocitySample.location.x) / duration,
            y: (currentVelocitySample.location.y - previousVelocitySample.location.y) / duration
        )
        guard let sourceView = self.view,
              let targetView = view,
              sourceView !== targetView else {
            return velocity
        }
        let origin = targetView.convert(CGPoint.zero, from: sourceView)
        let endpoint = targetView.convert(velocity, from: sourceView)
        return CGPoint(x: endpoint.x - origin.x, y: endpoint.y - origin.y)
    }

    private func recordVelocitySample(location: CGPoint, timestamp: TimeInterval) {
        if let currentVelocitySample,
           timestamp - currentVelocitySample.timestamp < 1.0 / 240.0 {
            self.currentVelocitySample = (location, timestamp)
            return
        }
        previousVelocitySample = currentVelocitySample
        currentVelocitySample = (location, timestamp)
    }
}
