...
public final class AetherKeyboardManager {
    ...
    public static func state(
        ...
        if let screen = window.windowScene?.screen {
            frameInWindow = window.convert(screenFrame, from: screen.coordinateSpace)
        } else {
            frameInWindow = window.convert(screenFrame, from: nil)
        }
        ...
        let isDockedToWindowBottom = !intersection.isNull
            ...
            && frameInWindow.maxY >= window.bounds.maxY - 1.0
        let height = isDockedToWindowBottom ? max(0.0, intersection.height) : 0.0
