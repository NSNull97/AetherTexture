...
private func layoutMetrics(for size: CGSize, traitCollection: UITraitCollection? = nil) -> LayoutMetrics {
    let widthClass: LayoutMetrics.WidthClass
    ...
    case .compact:
        widthClass = .compact
    default:
        widthClass = size.width > 690.0 && size.height > 650.0 ? .regular : .compact
    }
...
public final class AetherNativeWindow: UIWindow, AetherWindowHost {
    ...
    private var forceInCallStatusBarView: AetherTextNodeOverlayView?
    ...
    private var nodePresentationControllers: [ObjectIdentifier: AetherScreenController] = [:]
    ...
    private var environmentLayoutRefreshScheduled = false
        ...
        didSet {
            ...
            if let controller = contentController {
                ...
                if !windowLayout.size.width.isZero && !windowLayout.size.height.isZero {
                    ...
                    if isFirstLayout || updatingLayout != nil {
                        setNeedsLayout()
                    } else {
    ...
    /// Node-first root content. Internally hosted by a private
    /// `AetherScreenController` boundary so UIKit remains a window/lifecycle
    /// detail rather than the public composition primitive.
    public var contentNode: AetherScreenNode? {
        get {
            (contentController as? AetherScreenController)?.screenNode
        }
        set {
            if let newValue {
                contentController = AetherScreenController(screenNode: newValue)
            } else {
    ...
    @discardableResult
    public func presentInGlobalOverlay(
        _ node: AetherScreenNode,
        animated: Bool = true,
        completion: (() -> Void)? = nil
    ) -> AetherScreenController {
        let id = ObjectIdentifier(node)
        let controller = nodePresentationControllers[id] ?? AetherScreenController(screenNode: node)
        nodePresentationControllers[id] = controller
        presentInGlobalOverlay(controller, animated: animated, completion: completion)
        return controller
    }

    public func dismissGlobalOverlay(
        _ node: AetherScreenNode,
        animated: Bool = true,
        completion: (() -> Void)? = nil
    ) {
        let id = ObjectIdentifier(node)
        guard let controller = nodePresentationControllers[id] else {
            completion?()
            return
        }
        dismissGlobalOverlay(controller, animated: animated) { [weak self] in
            self?.nodePresentationControllers[id] = nil
            completion?()
        }
    }
    ...
    public func present(
        ...
        let controller = nodePresentationControllers[id] ?? AetherScreenController(screenNode: node)
        nodePresentationControllers[id] = controller
        present(controller, on: level, blockInteraction: blockInteraction, completion: completion)
        return controller
    }
    ...
    public func dismiss(_ node: AetherScreenNode, completion: (() -> Void)? = nil) {
        ...
        if topLevelOverlayControllers.contains(where: { $0 === controller }) {
            dismissGlobalOverlay(controller, animated: true) { [weak self] in
                self?.nodePresentationControllers[id] = nil
                completion?()
            }
        } else {
            presentationContext.dismiss(controller) { [weak self] in
                self?.nodePresentationControllers[id] = nil
                completion?()
            }
        }
    ...
    public func updateStatusBar(style: UIStatusBarStyle, hidden: Bool, transition: ContainedViewLayoutTransition) {
        let visibilityChanged = statusBarHidden != hidden
        statusBarHidden = hidden
        _rootController.updateStatusBar(style: style, hidden: hidden, transition: transition)
        if visibilityChanged {
            let height = hidden ? nil : windowScene?.statusBarManager?.statusBarFrame.height
            updateLayout {
                $0.updateStatusBarHeight(height, transition: transition, overrideTransition: true)
            }
        }
    }
    ...
    override public init(windowScene: UIWindowScene) {
        let boundsSize = windowScene.coordinateSpace.bounds.size
        ...
        self.windowLayout = WindowLayout(
            size: boundsSize,
            metrics: layoutMetrics(for: boundsSize, traitCollection: windowScene.traitCollection),
    ...
    override public func layoutSubviews() {
        ...
        let safeInsets = _rootController.view.safeAreaInsets == .zero
            ? self.safeAreaInsets
            : _rootController.view.safeAreaInsets
        let metrics = layoutMetrics(for: bounds.size, traitCollection: _rootController.traitCollection)
        if safeInsets != (updatingLayout?.layout.safeInsets ?? windowLayout.safeInsets)
            || metrics != (updatingLayout?.layout.metrics ?? windowLayout.metrics) {
            updateLayout {
                $0.updateSize(
                    self.bounds.size,
                    metrics: metrics,
    ...
    private func setupRootControllerCallbacks() {
        _rootController.transitionToSize = { [weak self] size, duration, _ in
            ...
            self.updateLayout {
                $0.updateSize(size, metrics: layoutMetrics(for: size, traitCollection: self._rootController.traitCollection), safeInsets: safeInsets, transition: transition, overrideTransition: true)
            }
        ...
        _rootController.environmentLayoutChanged = { [weak self] in
            guard let self else { return }
            let safeInsets = self._rootController.view.safeAreaInsets == .zero
                ? self.safeAreaInsets
                : self._rootController.view.safeAreaInsets
            let size = self.bounds.size
            self.updateLayout {
                $0.updateSize(
                    size,
                    metrics: layoutMetrics(for: size, traitCollection: self._rootController.traitCollection),
                    safeInsets: safeInsets,
                    transition: .immediate,
                    overrideTransition: false
                )
            }
            self.scheduleEnvironmentLayoutRefresh()
        }
    }

    /// Coalesces trait/safe-area callbacks and re-dispatches the current
    /// container layout after UIKit has finished propagating the environment.
    /// A content-size-category change does not alter `ContainerViewLayout`, so
    /// the normal equality-gated window commit would otherwise skip children.
    private func scheduleEnvironmentLayoutRefresh() {
        guard !environmentLayoutRefreshScheduled else { return }
        environmentLayoutRefreshScheduled = true
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            // UIKit can deliver the final geometry one turn after the trait
            // callback when decreasing Dynamic Type. Wait for that pass so a
            // transient zero-sized Texture hierarchy is never committed.
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                self.environmentLayoutRefreshScheduled = false
                self.layoutIfNeeded()

                let layout = containedLayout(from: self.windowLayout)
                if let controller = self.contentController {
                    controller.view.frame = CGRect(origin: .zero, size: self.windowLayout.size)
                    self.updateContentController(layout: layout, transition: .immediate)
                }
                for controller in self.topLevelOverlayControllers {
                    self.updateOverlayController(controller, layout: layout, transition: .immediate)
                }
            }
        }
    }
    ...
    private func updateForceInCallStatusBarView(text: String?, transition: ContainedViewLayoutTransition) {
        if let text {
            ...
            if let current = forceInCallStatusBarView {
                ...
                let current = AetherTextNodeOverlayView()
                current.backgroundColor = UIColor.systemGreen.withAlphaComponent(0.92)
                current.textColor = .white
                current.textAlignment = .center
                current.font = UIFont.aetherScaledSystemFont(ofSize: 13.0, weight: .semibold)
                current.maximumNumberOfLines = 1
                current.lineBreakMode = .byTruncatingTail
                current.verticalTextAlignment = .center
                current.alpha = 0.0
    ...
    private func handleSizeUpdate(_ size: CGSize) {
        ...
        updateLayout {
            $0.updateSize(size, metrics: layoutMetrics(for: size, traitCollection: self._rootController.traitCollection), safeInsets: safeInsets, transition: .immediate, overrideTransition: false)
        }
    }
    ...
    private func handleKeyboardFrameChange(_ notification: Notification) {
        ...
        let screenFrame = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue ?? .zero
        if isTablet && screenFrame.isEmpty {
            return
        }
        let keyboardFrame: CGRect
        if let screen = windowScene?.screen {
            keyboardFrame = convert(screenFrame, from: screen.coordinateSpace)
        } else {
            keyboardFrame = convert(screenFrame, from: nil)
        }
        let intersection = bounds.intersection(keyboardFrame)
        let isDockedToWindowBottom = !intersection.isNull
            && intersection.height > 0.0
            && keyboardFrame.maxY >= bounds.maxY - 1.0
        let keyboardHeight = isDockedToWindowBottom
            ? max(0.0, bounds.maxY - max(bounds.minY, keyboardFrame.minY))
            : 0.0
    ...
    private func handleKeyboardDidHide() {
        guard isCompletingInteractiveKeyboardDismissal else { return }
    ...
    private func beginInteractiveKeyboardDismissalCleanup() {
        ...
        // Keep the full interactive offset in place while UIKit performs its
        // own hide. Resetting it before keyboardDidHide makes the remote
        // surface jump back on-screen for a frame.
        _rootController.view.endEditing(true)

        let workItem = DispatchWorkItem { [weak self] in
            guard let self, self.isCompletingInteractiveKeyboardDismissal else { return }
    ...
    private func finishInteractiveKeyboardDismissalCleanup() {
        ...
        // The host view can be safely restored only after the keyboard is no
        // longer visible. This prevents the fast up/down flash on dismissal.
        ...
        guard windowLayout.inputHeight != nil || windowLayout.upperKeyboardInputPositionBound != nil else {
            return
        }
        updateLayout {
    ...
    private func updateLayout(_ update: (inout UpdatingLayout) -> Void) {
        if updatingLayout == nil {
            ...
            guard var pending = updatingLayout else { return }
            update(&pending)
            updatingLayout = pending
            setNeedsLayout()
        }
    ...
    private func commitUpdatingLayout() {
        ...
        var statusBarHeight: CGFloat? = statusBarHidden ? nil : windowScene?.statusBarManager?.statusBarFrame.height
        ...
        windowLayout = WindowLayout(
            ...
            metrics: layoutMetrics(for: pending.layout.size, traitCollection: _rootController.traitCollection),
            statusBarHeight: statusBarHeight,
        ...
        let runtimeLayout = AetherWindowLayout(
            ...
            verticalSizeClass: _rootController.traitCollection.verticalSizeClass,
        ...
        let shouldDeferOffsetReset = isCompletingInteractiveKeyboardDismissal
            && updatedInputOffset.isZero
            && pending.layout.inputHeight == nil
        if !previousInputOffset.isEqual(to: updatedInputOffset), !shouldDeferOffsetReset {
