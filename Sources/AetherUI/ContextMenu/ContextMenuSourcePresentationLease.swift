...
final class SourcePresentationLease {
    ...
    fileprivate static func makeProxyView(
        from contentView: UIView,
        positionedIn visualView: UIView
    ) -> UIView {
        let proxy = UIView(frame: CGRect(origin: .zero, size: visualView.bounds.size))
        ...
        contentProxy.frame = frameInVisualView.offsetBy(
            ...
            dy: -visualView.bounds.minY
        )
        proxy.addSubview(contentProxy)
        return proxy
    }
...
extension ContextMenuSourceDescriptor {
    init?(
        ...
        self.makeProxyView = { [weak visualView] in
            ...
            if let contentOnlyProxySource {
                return SourcePresentationLease.makeProxyView(
                    from: contentOnlyProxySource,
                    positionedIn: visualView
                )
            }
