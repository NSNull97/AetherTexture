...
#if APPSTORE_SAFE
/// App Store-safe placeholder. Callers already implement a public
/// transform/corner-radius morph when the SDF filter is unavailable, so the
/// failable initializer deliberately selects that path at compile time.
@available(iOS 26.0, *)
final class LensSDFFilter {
    init?() { return nil }

    func install(on target: CALayer, size: CGSize, cornerRadius: CGFloat, preserveExistingFilters: Bool = false) {}
    func uninstall() {}
