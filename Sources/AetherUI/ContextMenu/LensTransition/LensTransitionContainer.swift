import Foundation
import UIKit
// AetherUI port — `Display`, `ComponentFlow`, `UIKitRuntimeUtils`, and
// `GlassBackgroundComponent` are all replaced by symbols already vended from
// AetherUI itself (this same target). The lens transition is otherwise
// kept verbatim from Telegram's `LensTransition` submodule so timing / SDF
// parameters / private CA filter wiring remain identical.
...
private func createObject(className: String) -> NSObject? {
    guard AetherPrivateRuntime.isEnabled else { return nil }
...
private func setFilterName(object: NSObject, name: String) {
    guard AetherPrivateRuntime.isEnabled else { return }
...
public protocol LensTransitionContainerEffectView: UIView {
    ...
    func updatePosition(position: CGPoint, transition: ContainedViewLayoutTransition)
    func updateCornerRadius(duration: Double, keyframes: [CGFloat])
    func setTransitionFraction(value: CGFloat, duration: Double)
}

/// Internal companion capability used to resolve a source lens against its
/// owning transition before either side allocates a renderer. Keeping this
/// separate preserves compatibility for external effect-view conformers.
protocol LensTransitionAppearanceManagedEffectView: AnyObject {
...
final class LensTransitionContainerImpl: UIView, LensTransitionContainerProtocol {
    ...
    public func update(size: CGSize, cornerRadius: CGFloat, isDark: Bool, transition: ContainedViewLayoutTransition) {
...
private final class LensTransitionContainerFallbackImpl: UIView, LensTransitionContainerProtocol {
    ...
    init(frame: CGRect, appearanceStyle: AetherAppearanceStyle) {
        // Resolve the appearance before GlassBackgroundView allocates its
        // renderer. In particular, Legacy must start directly on the public
        // systemChromeMaterial implementation rather than briefly creating a
        // Liquid renderer and replacing it afterwards.
    ...
    fileprivate var usesLegacySurfaceRendererForTesting: Bool {
        backgroundView.usesLegacySurfaceRendererForTesting
    }

    fileprivate var usesAnyGlassRendererForTesting: Bool {
        backgroundView.usesAnyGlassRendererForTesting
    }

    fileprivate var legacyBlurStyleForTesting: UIBlurEffect.Style? {
        backgroundView.legacyBlurStyleForTesting
    }

    /// Classic/public fallback morph. Used by Legacy on every OS and by
    /// Liquid appearances where the SDF implementation is unavailable. No
    /// displacement effect is involved: the surface follows ordinary size,
    /// position and corner-radius keyframes.
    func animateIn(fromRect: CGRect, toRect: CGRect, fromCornerRadius: CGFloat, toCornerRadius: CGFloat, isDark: Bool, sourceEffectView: LensTransitionContainerEffectView) {
...
public final class LensTransitionContainer: UIView, AetherAppearanceConsumer {
    /// `nil` follows the application runtime. A non-`nil` value pins this
    /// container to that renderer generation for its entire lifetime.
    public let appearanceStyle: AetherAppearanceStyle?

    private let sourceEffectView: LensTransitionContainerEffectView
    private let publicContentsView: UIView
    private var impl: (UIView & LensTransitionContainerProtocol)
    ...
    public init(
        ...
        self.sourceEffectView = effectView
        self.publicContentsView = UIView()
        self.appliedAppearanceStyle = resolvedAppearanceStyle
        self.impl = Self.makeImplementation(
        ...
        publicContentsView.backgroundColor = .clear
        publicContentsView.clipsToBounds = true
        publicContentsView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        impl.contentsView.addSubview(publicContentsView)
        addSubview(impl)
        AetherAppearanceConsumerRegistry.register(self)
    }

    private static func makeImplementation(
        effectView: LensTransitionContainerEffectView,
        appearanceStyle: AetherAppearanceStyle
    ) -> UIView & LensTransitionContainerProtocol {
        applyAppearanceStyle(appearanceStyle, to: effectView)

        // After resolving the source, this style check is intentionally the
        // first renderer-producing branch. The Legacy path must never
        // initialize LensTransitionContainerImpl because its initializer
        // eagerly asks Core Animation for private SDF layers and a glass
        // displacement effect.
        guard appearanceStyle.usesLiquidGlass else {
            return LensTransitionContainerFallbackImpl(
                frame: .zero,
        ...
        #else
        if #available(iOS 26.0, *) {
            return LensTransitionContainerImpl(effectView: effectView)
        } else {
            return LensTransitionContainerFallbackImpl(
                frame: .zero,
                appearanceStyle: appearanceStyle
    ...
    override public func layoutSubviews() {
        ...
        impl.frame = bounds
        publicContentsView.frame = impl.contentsView.bounds
    }
    
    public func animateIn(fromRect: CGRect, toRect: CGRect, fromCornerRadius: CGFloat, toCornerRadius: CGFloat, isDark: Bool, sourceEffectView: LensTransitionContainerEffectView) {
        Self.applyAppearanceStyle(appliedAppearanceStyle, to: sourceEffectView)
    ...
    func aetherApplyAppearance(_ appearance: AetherAppearance, animated: Bool) {
        let resolvedAppearanceStyle = appearanceStyle ?? appearance.style
        Self.applyAppearanceStyle(resolvedAppearanceStyle, to: sourceEffectView)
        guard resolvedAppearanceStyle != appliedAppearanceStyle else { return }

        appliedAppearanceStyle = resolvedAppearanceStyle
        rebuildImplementation(for: resolvedAppearanceStyle)
    }

    private func rebuildImplementation(for appearanceStyle: AetherAppearanceStyle) {
        // In particular, tear the source effect down before releasing the old
        // Liquid implementation so no hidden UIGlassEffect/private blur state
        // survives while the public Legacy implementation is installed.
        Self.applyAppearanceStyle(appearanceStyle, to: sourceEffectView)
        let previousImplementation = impl
        Self.cancelAnimationsRecursively(in: previousImplementation)

        let replacement = Self.makeImplementation(
            effectView: sourceEffectView,
            appearanceStyle: appearanceStyle
        )
        replacement.frame = bounds

        publicContentsView.removeFromSuperview()
        replacement.contentsView.addSubview(publicContentsView)

        previousImplementation.removeFromSuperview()
        impl = replacement
        addSubview(replacement)

        if let lastUpdate {
            replacement.update(
                size: lastUpdate.size,
                cornerRadius: lastUpdate.cornerRadius,
                isDark: lastUpdate.isDark,
                transition: .immediate
            )
        }
        publicContentsView.frame = replacement.contentsView.bounds
        setNeedsLayout()
    }

    private static func applyAppearanceStyle(
        _ appearanceStyle: AetherAppearanceStyle,
        to effectView: LensTransitionContainerEffectView
    ) {
        (effectView as? LensTransitionAppearanceManagedEffectView)?
            .aetherApplyOwningAppearanceStyle(appearanceStyle)
    }

    private static func cancelAnimationsRecursively(in view: UIView) {
        view.layer.removeAllAnimations()
        for sublayer in view.layer.sublayers ?? [] {
            cancelAnimationsRecursively(in: sublayer)
        }
        for subview in view.subviews {
            cancelAnimationsRecursively(in: subview)
        }
    }

    private static func cancelAnimationsRecursively(in layer: CALayer) {
        layer.removeAllAnimations()
    ...
    internal var implementationViewForTesting: UIView {
        impl
    }

    internal var usesClassicImplementationForTesting: Bool {
        impl is LensTransitionContainerFallbackImpl
    }

    internal var usesPrivateSDFImplementationForTesting: Bool {
        #if APPSTORE_SAFE
        return false
        #else
        if #available(iOS 26.0, *) {
            return impl is LensTransitionContainerImpl
        }
        return false
        #endif
    }

    internal var classicImplementationUsesLegacySurfaceForTesting: Bool {
        (impl as? LensTransitionContainerFallbackImpl)?
            .usesLegacySurfaceRendererForTesting ?? false
    }

    internal var classicImplementationUsesAnyGlassRendererForTesting: Bool {
        (impl as? LensTransitionContainerFallbackImpl)?
            .usesAnyGlassRendererForTesting ?? false
    }

    internal var classicImplementationBlurStyleForTesting: UIBlurEffect.Style? {
        (impl as? LensTransitionContainerFallbackImpl)?
            .legacyBlurStyleForTesting
    }
}
...
// other AetherUI files (the context-menu controller) can spin up a
// matching displacement filter without re-implementing the runtime dance.

internal func aether_makeSDFObject(_ className: String) -> NSObject? {
    #if APPSTORE_SAFE
    return nil
    #else
...
internal func aether_makeCAFilter(name: String) -> NSObject? {
    #if APPSTORE_SAFE
    return nil
    #else
...
internal func aether_setCAFilterName(_ object: NSObject, _ name: String) {
    #if !APPSTORE_SAFE
