...
private func contextMenuReferenceOpeningGeometry(
    ...
    let egg = CGRect(
        ...
        y: source.midY - eggHeight * 0.5 + directionY * source.height * 0.34,
        width: diameter, height: eggHeight)
    ...
    // the source stays tighter while the leading belly rounds out. Keep the
    ...
    let radii = ContextMenuBloomCornerRadii(
        topLeft: corner(x: 0, y: 0), topRight: corner(x: 1, y: 0),
        bottomLeft: corner(x: 0, y: 1), bottomRight: corner(x: 1, y: 1))
...
func contextMenuGlassmorphicGeometrySample(
    ...
    if direction == .opening && rawProgress > 0 && rawProgress < 1 {
        ...
        let neck = reduceMotion ? 0 : contextMenuBloomSmoothRange(shapeProgress, start: 0.16, end: 0.26)
            * (1 - contextMenuBloomSmoothRange(shapeProgress, start: 0.32, end: 0.52))
