...
final class ContextMenuActionsView: UIView {
    ...
    struct Metrics {
        ...
        let separatorHorizontalInset: CGFloat
        let automaticallySeparatesActionRows: Bool

        static func resolve(for appearanceStyle: AetherAppearanceStyle) -> Metrics {
            switch appearanceStyle {
            case .legacy:
                // iOS 18 Context Menu (Figma 754:62668). Classic rows run
                // edge-to-edge inside the 250pt platter; each row owns its
                // 16pt content padding and the platter adds no vertical pad.
                return Metrics(
                    cornerRadius: 12.0,
                    preferredWidth: 250.0,
                    contentInset: .zero,
                    separatorHorizontalInset: 0.0,
                    automaticallySeparatesActionRows: true
                )
            case .liquidGlassV1, .liquidGlassV2:
                return Metrics(
                    ...
                    separatorHorizontalInset: 4.0,
                    automaticallySeparatesActionRows: false
                )
            }
        }
    }
    ...
    func preferredSize(maxWidth: CGFloat) -> CGSize {
        let width = min(maxWidth, metrics.preferredWidth)
    ...
    internal var metricsForTesting: Metrics { metrics }

    internal var automaticActionSeparatorCountForTesting: Int {
        rowViews.reduce(into: 0) { result, entry in
            if let row = entry.view as? ContextMenuActionItemView,
               row.showsRowSeparator {
                result += 1
            }
        }
    }
    ...
    override func layoutSubviews() {
        ...
        // Liquid menus keep their padded platter layout; Legacy runs rows
        // edge-to-edge and lets each row own the Figma-specified 16pt inset.
    ...
    private func buildRowViews() {
        ...
        for (itemIndex, item) in items.enumerated() {
            ...
            case let .header(title):
                ...
                container.addSubview(label.view)
                label.view.snp.makeConstraints { make in
                    let inset = ContextMenuActionItemView.Metrics
                        .resolve(for: appearanceStyle)
                        .horizontalInset
            ...
            case let .action(action):
                let row = ContextMenuActionItemView(
                    item: action,
                    appearanceStyle: appearanceStyle
                )
                row.reservesLeadingSlot = reservesLeadingSlotForActionRows
                if metrics.automaticallySeparatesActionRows,
                   items.indices.contains(itemIndex + 1),
                   case .action = items[itemIndex + 1] {
                    row.showsRowSeparator = true
                }
                contentContainer.addSubview(row)
            ...
            case .separator:
                ...
                line.snp.makeConstraints { make in
                    make.leading.equalToSuperview().offset(metrics.separatorHorizontalInset)
                    make.trailing.equalToSuperview().offset(-metrics.separatorHorizontalInset)
    ...
    private func makeHeaderRow(title: String, chevronSymbol: String) -> UIView {
        ...
        chevron.view.isUserInteractionEnabled = false
        container.addSubview(chevron.view)

        let label = ASTextNode()
        label.attributedText = NSAttributedString(
            string: title,
            attributes: [
                .font: UIFont.aetherScaledSystemFont(ofSize: 17.0, weight: .semibold),
        ...
        chevron.view.snp.makeConstraints { make in
            make.leading.equalToSuperview().offset(
                ContextMenuActionItemView.Metrics.resolve(for: appearanceStyle).horizontalInset
            )
            make.centerY.equalToSuperview()
        ...
        label.view.snp.makeConstraints { make in
            make.leading.equalTo(chevron.view.snp.trailing).offset(10.0)
            make.trailing.lessThanOrEqualToSuperview().offset(
                -ContextMenuActionItemView.Metrics.resolve(for: appearanceStyle).horizontalInset
            )
            make.centerY.equalToSuperview()
        }
