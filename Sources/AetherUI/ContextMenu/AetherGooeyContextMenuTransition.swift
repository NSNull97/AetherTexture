...
public struct AetherGooeyContextMenuTransitionConfiguration {
    /// The canonical appearance generation this transition was configured for.
    ///
    /// Passing an explicit value pins the configuration to that style. Omitting
    /// it snapshots the runtime appearance when the configuration is created,
    /// so a later global appearance change cannot silently turn a classic
    /// transition into a Liquid Glass transition (or vice versa).
    public var appearanceStyle: AetherAppearanceStyle

    /// Whether the configured appearance is allowed to allocate the gooey
    /// morph renderer and its Metal/display-link resources.
    public var usesLiquidGlassTransition: Bool {
        appearanceStyle.usesLiquidGlass
    }
    ...
    public init(
        ...
        self.appearanceStyle = appearanceStyle ?? AetherAppearance.runtimeCurrent.style
    ...
    static func `default`(
        ...
        case .legacy:
            return .init(
                ...
                springDamping: 1.0,
                springResponse: 0.20,
                connectorMaximumLength: 0,
                connectorMinimumThickness: 0,
                connectorMaximumThickness: 0,
...
public final class AetherGooeyContextMenuTransition: AetherContextMenuTransitioning {
    ...
    private var classicAnimator: UIViewPropertyAnimator?
    private var classicPhase: AetherGooeyTransitionPhase?
    private weak var classicPresentedSourceView: UIView?
    private var classicPresentedSourceAlpha: CGFloat?
    private var classicPresentedSourceInteraction: Bool?
    ...
    deinit {
        animator?.cancel()
        if classicAnimator != nil {
            classicAnimator?.stopAnimation(true)
            cleanup?()
        }
    ...
    public func animateOpen(
        ...
        guard configuration.usesLiquidGlassTransition else {
            animateClassicOpen(
    ...
    public func cancel() {
        let cancelledClassicPhase = classicPhase
        animator?.cancel()
        animator = nil
        classicAnimator?.stopAnimation(true)
        classicAnimator = nil
        ...
        self.completion = nil
        completion?(false)
        if case .opening? = cancelledClassicPhase {
            clearClassicPresentedSourceState()
        }
        signpostID = nil
    }

    // MARK: Classic transition

    /// The Legacy path deliberately stays above the renderer allocation
    /// boundary. It never captures geometry, creates an overlay/morph surface,
    /// initializes Metal/SDF resources, or starts `AetherGooeyAnimator`'s
    /// display link.
    private func animateClassicOpen(
        sourceView: UIView,
        menuView: UIView,
        containerView: UIView,
        completion: @escaping (Bool) -> Void
        ...
        let settings = Self.accessibilitySettings(configuration: configuration)
        let duration = adjustedDuration(base: configuration.durationOpen, settings: settings)
        let animatables = Self.collectContentAnimatables(in: menuView)
        let menuInitialAlpha = menuView.alpha
        let menuTargetAlpha = menuInitialAlpha > 0.01 ? menuInitialAlpha : 1.0
        let menuOriginalFrame = menuView.frame
        ...
        let sourceOriginalAlpha = sourceView.alpha
        let sourceOriginalInteraction = sourceView.isUserInteractionEnabled
        let startTransform = settings.reduceMotion
            ? menuOriginalTransform
            : menuOriginalTransform.scaledBy(x: 0.96, y: 0.96)
        ...
        sourceView.layer.removeAllAnimations()
        menuView.frame = menuOriginalFrame
        menuView.transform = startTransform
        menuView.alpha = 0.0
        menuView.isUserInteractionEnabled = false
        sourceView.alpha = sourceOriginalAlpha
        sourceView.isUserInteractionEnabled = false
        ...
        cleanup = { [weak menuView, weak sourceView] in
            CATransaction.begin()
            CATransaction.setDisableActions(true)
            menuView?.layer.removeAllAnimations()
            menuView?.frame = menuOriginalFrame
        ...
        self.completion = completion
        classicPresentedSourceView = sourceView
        classicPresentedSourceAlpha = sourceOriginalAlpha
        classicPresentedSourceInteraction = sourceOriginalInteraction
        classicPhase = .opening
        ...
        classicAnimator = animator
        animator.addCompletion { [weak self, weak animator, weak menuView, weak sourceView] position in
            guard let self, let animator, self.classicAnimator === animator else { return }
            self.classicAnimator = nil
            self.classicPhase = nil
            ...
            if finished {
                ...
                sourceView?.alpha = sourceOriginalAlpha
                sourceView?.isUserInteractionEnabled = sourceOriginalInteraction
                self.clearClassicPresentedSourceState()
            }
            CATransaction.commit()
    ...
    private func animateClassicClose(
        sourceView: UIView?,
        menuView: UIView,
        containerView: UIView,
        completion: @escaping (Bool) -> Void
        ...
        let settings = Self.accessibilitySettings(configuration: configuration)
        let duration = adjustedDuration(base: configuration.durationClose, settings: settings)
        let animatables = Self.collectContentAnimatables(in: menuView)
        let menuOriginalAlpha = menuView.alpha
        let menuOriginalFrame = menuView.frame
        let menuOriginalTransform = menuView.transform
        ...
        let sourceOriginalAlpha = sourceView?.alpha
        let sourceOriginalInteraction = sourceView?.isUserInteractionEnabled
        let restoresPresentedSource = sourceView != nil && classicPresentedSourceView === sourceView
        let sourceTargetAlpha = restoresPresentedSource
            ? classicPresentedSourceAlpha
            : sourceOriginalAlpha.map { _ in 1.0 }
        let sourceTargetInteraction = restoresPresentedSource
            ? classicPresentedSourceInteraction
            : sourceOriginalInteraction
        let targetTransform = settings.reduceMotion
            ? menuOriginalTransform
            : menuOriginalTransform.scaledBy(x: 0.96, y: 0.96)
        ...
        self.completion = completion
        classicPhase = .closing

        let animator = UIViewPropertyAnimator(duration: duration, curve: .easeInOut) {
            menuView.frame = menuOriginalFrame
            menuView.transform = targetTransform
            menuView.alpha = 0.0
            if let sourceTargetAlpha {
        ...
        animator.addCompletion { [weak self, weak animator, weak menuView, weak sourceView] position in
            ...
            if finished {
                ...
                if let sourceTargetInteraction {
                    sourceView?.isUserInteractionEnabled = sourceTargetInteraction
                }
                animatables.forEach { $0.finishGooeyClose() }
                self.clearClassicPresentedSourceState()
            } else {
    ...
    private func clearClassicPresentedSourceState() {
        classicPresentedSourceView = nil
        classicPresentedSourceAlpha = nil
        classicPresentedSourceInteraction = nil
    }

    var resolvedAppearanceStyleForTesting: AetherAppearanceStyle {
        configuration.appearanceStyle
    }

    var hasClassicAnimatorForTesting: Bool {
        classicAnimator != nil
    }

    var hasLiquidTransitionResourcesForTesting: Bool {
        overlayView != nil || animator != nil
    }

    var hasGooeyDisplayLinkForTesting: Bool {
        animator?.hasActiveDisplayLinkForTesting == true
    }
...
final class AetherGooeyDebugView: UIView {
    ...
    private let label = AetherTextNodeOverlayView()
    ...
    override init(frame: CGRect) {
        ...
        label.textInsets = UIEdgeInsets(top: 2.0, left: 4.0, bottom: 2.0, right: 4.0)
        addSubview(label)
    }
