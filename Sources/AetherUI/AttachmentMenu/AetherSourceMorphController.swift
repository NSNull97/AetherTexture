...
public final class AetherSourceMorphController: NSObject, AetherAppearanceConsumer {
    public struct Configuration {
        ...
        public init(
            ...
            openDuration: TimeInterval = AetherMotion.contextMenu.presentation.duration,
            closeDuration: TimeInterval = AetherMotion.contextMenu.dismissal.duration
        ) {
    ...
    /// A non-`nil` value is a local, pinned renderer selection. Passing
    /// `nil` keeps the controller inherited from the application runtime.
    ...
    public func present(
        ...
        // Resolve the generation before allocating an overlay, material
        // renderer, source proxy or animation driver. In particular an
        // explicit Legacy controller must never briefly create Liquid Glass.
        ...
        let sourceSnapshot = resolvedAppearanceStyle.usesLiquidGlass
            ? Self.makeProxyView(from: sourceView)
            : nil
    ...
    public func dismiss(animated: Bool = true) {
        ...
        surfaceView.close(duration: animated ? configuration.closeDuration : 0.0) { [weak self] in
            self?.finishDismissal()
        }
    }

    private func finishDismissal(synchronously: Bool = false) {
        guard isPresented || overlayView != nil || surfaceView != nil else { return }
        isPresented = false
        surfaceView?.cancelAnimations()
        ...
        // The Liquid path keeps its proxy over the restored source for one
        // committed frame. Runtime generation changes, and the classic
        // Legacy popup which has no proxy, tear down synchronously.
        if synchronously || presentedAppearanceStyle == .legacy {
    ...
    private func performPendingCleanup() {
        let cleanup = pendingCleanup
        pendingCleanup = nil
        cleanup?()
    }

    /// Generation-specific transition objects cannot be swapped safely in
    /// flight. End an inherited presentation synchronously, then let the next
    /// `present` create a clean renderer in the new generation. Explicit
    /// local styles remain pinned.
    func aetherApplyAppearance(_ appearance: AetherAppearance, animated _: Bool) {
        inheritedAppearance = appearance
        guard appearanceStyle == nil,
              let presentedAppearanceStyle,
              presentedAppearanceStyle != appearance.style else { return }
        finishDismissal(synchronously: true)
    }
    ...
    internal var hasLiquidTransitionResourcesForTesting: Bool {
        surfaceView?.hasLiquidTransitionResourcesForTesting == true
    }

    internal var hasActiveDisplayLinkForTesting: Bool {
        surfaceView?.hasActiveDisplayLinkForTesting == true
    }

    internal var hasSourceSnapshotForTesting: Bool {
        surfaceView?.hasSourceSnapshotForTesting == true
    }
    ...
    internal var contentViewForTesting: UIView {
        contentView
    }
}

public final class AetherAttachmentMenuController: NSObject, AetherAppearanceConsumer {
    public struct Item {
    ...
    /// A local renderer override. `nil` follows the application runtime.
    public let appearanceStyle: AetherAppearanceStyle?
    ...
    public func present(from sourceView: UIView, in hostView: UIView? = nil) {
        ...
        if appearanceStyle == nil {
            // Preserve the parent's latest inherited seed even when tests or
            // embedders drive the consumer registry directly without also
            // changing the thread-local runtime snapshot.
            controller.aetherApplyAppearance(inheritedAppearance, animated: false)
        }
    ...
    func aetherApplyAppearance(_ appearance: AetherAppearance, animated: Bool) {
        inheritedAppearance = appearance
        guard appearanceStyle == nil else { return }
        // The child is also a consumer, but forwarding here guarantees the
        // owning attachment controller releases its self-retain in this same
        // synchronous runtime update, independent of registry ordering.
        morphController?.aetherApplyAppearance(appearance, animated: animated)
    }

    internal var resolvedAppearanceStyleForTesting: AetherAppearanceStyle {
        morphController?.resolvedAppearanceStyleForTesting
            ?? appearanceStyle
            ?? inheritedAppearance.style
    }

    internal var hasActivePresentationForTesting: Bool {
        morphController?.hasPresentationOverlayForTesting == true
    }

    internal var hasLiquidTransitionResourcesForTesting: Bool {
        morphController?.hasLiquidTransitionResourcesForTesting == true
    }

    internal var legacyBlurStyleForTesting: UIBlurEffect.Style? {
        morphController?.legacyBlurStyleForTesting
    }
}
...
private final class SourceMorphSurfaceView: UIView {
    ...
    private let appearanceStyle: AetherAppearanceStyle

    private let shadowView = UIView()
    private let glassView: GlassBackgroundView?
    private let legacyBlurView: UIVisualEffectView?
    private let materialContentView: UIView
    ...
    init(
        ...
        if appearanceStyle.usesLiquidGlass {
            let glassView = GlassBackgroundView(
                style: .regular,
                appearanceStyle: appearanceStyle
            )
            self.glassView = glassView
            self.legacyBlurView = nil
            self.materialContentView = glassView.contentView
            ...
            self.materialContentView = blurView.contentView
        ...
        if let glassView {
            ...
            glassView.glassIsInteractive = false
            ...
            strokeView.layer.borderColor = UIColor.clear.cgColor
            strokeView.layer.borderWidth = 0
            ...
            shadowView.layer.shadowRadius = 18.0
            shadowView.layer.shadowOffset = CGSize(width: 0, height: 7.0)

            legacyBlurView.clipsToBounds = true
            legacyBlurView.layer.cornerCurve = .continuous
            addSubview(legacyBlurView)
        }

        contentHost.backgroundColor = .clear
        materialContentView.addSubview(contentHost)
        ...
        if let sourceSnapshot {
            ...
            materialContentView.addSubview(sourceSnapshot)
        }

        if appearanceStyle.usesLiquidGlass {
            contentHost.alpha = 0.0
            contentHost.transform = CGAffineTransform(translationX: 0, y: 10)
            applyLiquid(progress: 0.0)
        } else {
            applyLegacyGeometry()
            applyLegacyVisibility(isOpen: false)
        }
    }
    ...
    func open(duration: TimeInterval) {
        if appearanceStyle.usesLiquidGlass {
            startLiquidAnimation(from: 0.0, to: 1.0, duration: duration, completion: nil)
        } else {
            startLegacyAnimation(isOpening: true, duration: duration, completion: nil)
        }
    }

    func close(duration: TimeInterval, completion: @escaping () -> Void) {
        if appearanceStyle.usesLiquidGlass {
            startLiquidAnimation(from: progress, to: 0.0, duration: duration, completion: completion)
        } else {
            startLegacyAnimation(isOpening: false, duration: duration, completion: completion)
        }
    }
    ...
    private func startLegacyAnimation(
        isOpening: Bool,
        ...
        let animations = { [weak self] in
            self?.applyLegacyVisibility(isOpen: isOpening)
        }
    ...
    private func applyLegacyGeometry() {
        surfaceFrame = targetFrame
        shadowView.frame = targetFrame
        shadowView.layer.shadowPath = UIBezierPath(
            roundedRect: shadowView.bounds,
            cornerRadius: targetCornerRadius
        ).cgPath

        legacyBlurView?.frame = targetFrame
        legacyBlurView?.layer.cornerRadius = targetCornerRadius
        legacyBlurView?.layoutIfNeeded()
        contentHost.frame = CGRect(origin: .zero, size: targetFrame.size)
        for subview in contentHost.subviews {
            subview.frame = contentHost.bounds
        }
    }

    private func applyLegacyVisibility(isOpen: Bool) {
        let transform = isOpen
            ? CGAffineTransform.identity
            : CGAffineTransform(translationX: 0, y: 8.0).scaledBy(x: 0.96, y: 0.96)
        let alpha: CGFloat = isOpen ? 1.0 : 0.0

        shadowView.alpha = alpha
        shadowView.transform = transform
        legacyBlurView?.alpha = alpha
        legacyBlurView?.transform = transform
        contentHost.alpha = 1.0
    ...
    private func startLiquidAnimation(
        ...
        animationDuration = max(0.0, duration)
        animationStartTime = CACurrentMediaTime()
        animationCompletion = completion
        progress = animationFromProgress

        applyLiquid(progress: progress)
        guard animationDuration > 0, !UIAccessibility.isReduceMotionEnabled else {
    ...
    private func applyLiquid(progress rawProgress: CGFloat) {
        ...
        tintView.frame = glassView?.bounds ?? .zero
        tintView.layer.cornerRadius = cornerRadius
        tintView.layer.cornerCurve = .continuous

        strokeView.frame = glassView?.bounds ?? .zero
    ...
    var hasLiquidTransitionResourcesForTesting: Bool {
        glassView != nil || sourceSnapshot != nil || displayLink != nil
    }
    ...
    var hasSourceSnapshotForTesting: Bool {
        sourceSnapshot != nil
    }

    var legacyBlurStyleForTesting: UIBlurEffect.Style? {
...
private final class AttachmentMenuContentView: UIView {
    ...
    override func layoutSubviews() {
        ...
        // Source-morph content is briefly installed into a zero-sized host
        // before its target geometry is applied. Compact embedder bounds can
        // also be narrower than the menu's standard horizontal insets.
        // Never hand UIKit a negative row width during either state.
        let width = max(0, bounds.width - Self.horizontalPadding * 2.0)
...
private final class AttachmentMenuRowView: UIControl {
    private let contentNode: AttachmentMenuRowContentNode

    init(item: AetherAttachmentMenuController.Item) {
        self.contentNode = AttachmentMenuRowContentNode(item: item)
        super.init(frame: .zero)
        ...
        didSet {
            UIView.animate(
                ...
                self.backgroundColor = self.isHighlighted
                    ...
                    : .clear
                self.contentNode.setHighlighted(self.isHighlighted)
            }
        }
    }
...
private final class AttachmentMenuRowContentNode: ASDisplayNode {
    ...
    init(item: AetherAttachmentMenuController.Item) {
        super.init()
        iconContainerNode.backgroundColor = UIColor.secondarySystemFill.withAlphaComponent(0.56)
        iconContainerNode.cornerRadius = 24.0
        iconContainerNode.layer.cornerCurve = .continuous
        iconContainerNode.isUserInteractionEnabled = false
        addSubnode(iconContainerNode)

        imageNode.image = item.icon?.withRenderingMode(.alwaysTemplate)
        imageNode.tintColor = .label
        imageNode.contentMode = .scaleAspectFit
        imageNode.isUserInteractionEnabled = false
        iconContainerNode.addSubnode(imageNode)
        ...
        titleNode.attributedText = NSAttributedString(
            string: item.title,
            attributes: [
                .font: UIFont.aetherScaledSystemFont(ofSize: 25.0, weight: .regular),
    ...
    func setHighlighted(_ highlighted: Bool) {
        iconContainerNode.transform = highlighted
            ? CATransform3DMakeScale(0.94, 0.94, 1.0)
    ...
    override func layout() {
        ...
        iconContainerNode.frame = CGRect(
            x: 0,
            y: (bounds.height - iconSide) * 0.5,
            width: iconSide,
            height: iconSide
        )
        imageNode.frame = iconContainerNode.bounds.insetBy(dx: 12, dy: 12)
        let titleX = min(74, max(0, bounds.width))
