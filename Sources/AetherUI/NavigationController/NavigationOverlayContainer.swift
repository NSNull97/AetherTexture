...
final class NavigationOverlayContainer: UIView {
    ...
    init(controller: AetherViewController, blocksInteractionUntilReady: Bool) {
    ...
    func transitionIn(animated: Bool, completion: (() -> Void)? = nil) {
        ...
        if animated {
            ...
            let profile = AetherMotion.sourceModal.presentation
            UIView.animate(withDuration: profile.duration, delay: 0.0, usingSpringWithDamping: profile.dampingRatio, initialSpringVelocity: profile.initialVelocity, options: [.beginFromCurrentState, .allowUserInteraction], animations: {
                self.alpha = 1.0
    ...
    func transitionOut(animated: Bool, completion: @escaping () -> Void) {
        ...
        if animated {
            let profile = AetherMotion.sourceModal.dismissal
            UIView.animate(withDuration: profile.duration, delay: 0.0, usingSpringWithDamping: profile.dampingRatio, initialSpringVelocity: profile.initialVelocity, options: [.beginFromCurrentState, .allowUserInteraction], animations: animations, completion: completed)
        } else {
            animations()
            completed(true)
        }
