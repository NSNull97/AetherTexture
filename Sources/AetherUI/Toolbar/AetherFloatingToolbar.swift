...
public final class AetherFloatingToolbarView: UIView {
    ...
    /// Manual segment container. We intentionally avoid `UIStackView` here:
    /// segment internals render through nodes / glass controls, and the
    /// toolbar only needs a tiny deterministic horizontal layout pass.
    private let stackContainerView = UIView()

    private var segmentViews: [UIView & ToolbarSegmentView] = []
    private var segmentFlexFlags: [Bool] = []
    ...
    private func rebuild() {
        ...
        for segment in segments {
            ...
            switch segment {
            case .search, .spacer:
                segmentFlexFlags.append(true)
            case .pill, .standalone:
                segmentFlexFlags.append(false)
            }
        }
    ...
    override public func layoutSubviews() {
        ...
        guard !segmentViews.isEmpty else {
            stackContainerView.frame = .zero
            return
        }

        let availableWidth = max(0.0, bounds.width - sideInset * 2.0)
        let spacingTotal = segmentSpacing * CGFloat(max(0, segmentViews.count - 1))
        let fixedWidths = segmentViews.enumerated().map { index, view -> CGFloat in
            guard !segmentFlexFlags[index] else { return 0.0 }
            return ceil(view.systemLayoutSizeFitting(UIView.layoutFittingCompressedSize).width)
        }
        let fixedTotal = fixedWidths.reduce(0.0, +)
        let flexCount = segmentFlexFlags.filter { $0 }.count
        let hasFlex = flexCount > 0
        let flexWidth = hasFlex ? max(0.0, (availableWidth - fixedTotal - spacingTotal) / CGFloat(flexCount)) : 0.0
        let contentWidth = hasFlex ? availableWidth : min(availableWidth, fixedTotal + spacingTotal)
        let originX = hasFlex ? sideInset : max(sideInset, floor((bounds.width - contentWidth) / 2.0))
        let originY = floor((bounds.height - pillHeight) / 2.0)

        stackContainerView.frame = CGRect(x: originX, y: originY, width: contentWidth, height: pillHeight)

        var x: CGFloat = 0.0
        for (index, view) in segmentViews.enumerated() {
            let width = segmentFlexFlags[index] ? flexWidth : fixedWidths[index]
            view.frame = CGRect(x: x, y: 0.0, width: width, height: pillHeight)
            x += width
            if index < segmentViews.count - 1 {
                x += segmentSpacing
            }
        }
    }
...
private final class PillSegmentView: UIView, ToolbarSegmentView {
    ...
    private var lastRebuildSize: CGSize = .zero
    ...
    override func layoutSubviews() {
        super.layoutSubviews()
        if needsGroupRebuild || lastRebuildSize != bounds.size {
            rebuildGroup()
        }
    }
...
private final class SearchSegmentView: UIView, ToolbarSegmentView, UITextFieldDelegate {
    ...
    init(config: AetherFloatingToolbarView.SearchConfig) {
        ...
        self.iconNode.image = config.icon?.withRenderingMode(.alwaysTemplate)
    ...
    private func setupConstraints() {
        ...
        let hasIcon = iconNode.image != nil
        ...
        iconNode.view.snp.makeConstraints { make in
            make.leading.equalToSuperview().offset(horizontalInset)
            make.centerY.equalToSuperview()
            make.height.equalTo(iconSize)
            make.width.equalTo(hasIcon ? iconSize : 0)
        }
        iconNode.isHidden = !hasIcon

        textField.snp.makeConstraints { make in
            if hasIcon {
                make.leading.equalTo(iconNode.view.snp.trailing).offset(iconGap)
            } else {
                make.leading.equalToSuperview().offset(horizontalInset)
            }
    ...
    func applyTheme(_ theme: AetherFloatingToolbarView.Theme, isDark _: Bool) {
        iconNode.tintColor = theme.placeholderColor
