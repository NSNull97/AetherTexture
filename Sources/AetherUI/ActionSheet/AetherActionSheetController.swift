...
open class AetherActionSheetController: ASDKViewController<ASDisplayNode> {
    ...
    private var groupOverlayNodes: [Int: ASDisplayNode] = [:]
    ...
    public init(theme: AetherActionSheetTheme = .light) {
        ...
        super.init(node: ASDisplayNode(viewBlock: {
            AetherActionSheetControllerView(theme: rootTheme)
        }))
    ...
    public func setItemGroupOverlayNode(groupIndex: Int, node: ASDisplayNode?) {
        groupOverlayNodes[groupIndex] = node
        setItemGroupOverlayView(groupIndex: groupIndex, view: node?.view)
    }
...
final class AetherActionSheetControllerView: UIView {
    ...
    func setGroups(_ groups: [AetherActionSheetItemGroup]) {
        ...
        hasSetInitialContentOffset = false
        setNeedsLayout()
    }
    ...
    override func layoutSubviews() {
        ...
        let totalHeight = y
        let topInset = max(safeInsets.top, Self.bottomPadding)
        let bottomInset = max(safeInsets.bottom, Self.bottomPadding)
        let availableHeight = max(0.0, size.height - topInset - bottomInset)
        let visibleHeight = min(totalHeight, availableHeight)
        scrollView.frame = CGRect(
            x: originX,
            y: size.height - bottomInset - visibleHeight,
        ...
        scrollView.alwaysBounceVertical = totalHeight > visibleHeight + 0.5
        if !hasSetInitialContentOffset {
            scrollView.contentOffset.y = max(0.0, totalHeight - visibleHeight)
    ...
    func animateIn(completion: @escaping () -> Void) {
        layoutIfNeeded()
        let travel = bounds.height - scrollView.frame.minY
        scrollView.transform = CGAffineTransform(translationX: 0, y: travel)
        dimView.alpha = 0.0
    ...
    func animateOut(cancelled: Bool) {
        ...
        UIView.animate(
            ...
            animations: {
                ...
                self.scrollView.transform = CGAffineTransform(translationX: 0, y: travel)
            },
            completion: { _ in
