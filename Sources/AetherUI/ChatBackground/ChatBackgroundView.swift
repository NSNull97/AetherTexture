...
public struct ChatBackgroundSettings {
    ...
    public struct RadialGradient {
        ...
        public var fields: [RadialField]

        public init(baseColor: UIColor, fields: [RadialField]) {
    ...
    public struct ImageOverlay {
        ...
        public var gradientTint: RadialGradient?

        public init(
            image: UIImage,
            mode: ImageContentMode = .aspectFill,
            alpha: CGFloat = 1.0,
            gradientTint: RadialGradient? = nil
        ) {
            self.image = image
            self.mode = mode
    ...
    public func estimatedContentStats(traitCollection: UITraitCollection = .current) -> ChatBackgroundContentStats {
        ...
        case let .radialGradient(gradient):
            return ChatBackgroundSettings.contentStats(
                for: [gradient.baseColor] + gradient.fields.map(\.color),
...
private final class LegacyChatBackgroundView: UIView {
    ...
    private func commonInit() {
        ...
        imageNode.view.clipsToBounds = true
        visualContainer.addSubview(imageNode.view)
        visualContainer.addSubview(tiledImageView)
        overlayImageNode.view.clipsToBounds = true
        visualContainer.addSubview(overlayImageNode.view)
    ...
    private func applyContent() {
        ...
        case let .radialGradient(gradient):
            visualContainer.backgroundColor = gradient.baseColor
    ...
    private func applyImageOverlay() {
        overlayImageNode.isHidden = true
        overlayImageNode.image = nil
        overlayImageNode.view.alpha = 1.0
        ...
        case .aspectFill, .aspectFit, .fill:
            ...
            overlayImageNode.view.alpha = overlay.alpha
            switch overlay.mode {
            case .aspectFill:
                overlayImageNode.contentMode = .scaleAspectFill
            case .aspectFit:
                overlayImageNode.contentMode = .scaleAspectFit
            case .fill:
                overlayImageNode.contentMode = .scaleToFill
            case .tile:
                break
            }
        }
    ...
    private func updateFrames(transition: ContainedViewLayoutTransition) {
        ...
        transition.updateFrame(view: imageNode.view, frame: contentBounds)
        transition.updateFrame(view: tiledImageView, frame: contentBounds)
        transition.updateFrame(view: overlayImageNode.view, frame: contentBounds)
...
@available(*, deprecated, message: "Use ChatBackgroundNode in node-first screens.")
public final class ChatBackgroundView: UIView {
    public let node: ChatBackgroundNode

    public var settings: ChatBackgroundSettings {
        node.settings
    }

    public var contentStats: ChatBackgroundContentStats {
        node.contentStats
    }

    public var contentStatsUpdated: ((ChatBackgroundContentStats) -> Void)? {
        get { node.contentStatsUpdated }
        set { node.contentStatsUpdated = newValue }
    }

    public init(settings: ChatBackgroundSettings = .telegramClassic) {
        self.node = ChatBackgroundNode(settings: settings)
        super.init(frame: .zero)
        commonInit()
    }

    public required init?(coder: NSCoder) {
        self.node = ChatBackgroundNode(settings: .telegramClassic)
        super.init(coder: coder)
        commonInit()
    }
    ...
    public func setSettings(
        ...
        node.setSettings(settings, transition: transition)
    }
