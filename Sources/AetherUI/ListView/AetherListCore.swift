...
/// by `AetherListNode` on the main thread after the corresponding node exists.
...
public struct AetherListIntermediateItem: Equatable {
    ...
    public init(stableId: AnyHashable, height: CGFloat) {
        ...
        self.height = AetherListFrameMetrics.sanitizedLength(height)
    }
}
...
/// tests. The live node still replays directly against mounted Texture nodes.
...
/// Immutable output from the pure transaction planner. `AetherListNode` owns
/// Texture replay, but index/remap math is centralized here so callers and
/// tests do not have to exercise mounted nodes for state-only validation.
...
/// UIKit-free transaction replay policy. `AetherListNode` performs the
/// actual node mutations, but the decision layer that chooses insertion,
...
internal enum AetherListAsyncLayoutCommandPlanner {
    static func commands<ItemID: Hashable>(
        ...
        let descriptorByIndex = Dictionary(
            itemDescriptors.lazy
                .filter { prefetchRange.contains($0.index) }
                .map { ($0.index, $0) },
...
internal enum AetherListEffectiveInsetsPlanner {
    static func plan(
        ...
        let scrollIndicatorInsets = explicitScrollIndicatorInsets.map(AetherListFrameMetrics.sanitizedInsets) ?? contentInset
    ...
    static func effectiveContentInset(
        ...
        var effective = AetherListFrameMetrics.sanitizedInsets(baseInsets)
        effective.bottom += AetherListFrameMetrics.sanitizedLength(keyboardBottomInset)
        if stackFromBottom {
            let availableHeight = normalized(viewportHeight) - effective.top - effective.bottom
            let topPadding = max(0, availableHeight - AetherListFrameMetrics.sanitizedLength(totalContentHeight))
            effective.top += topPadding
        }
        return AetherListFrameMetrics.sanitizedInsets(effective)
    }
    ...
    static func maxContentOffsetY(
        ...
        let contentInset = AetherListFrameMetrics.sanitizedInsets(contentInset)
        let minY = AetherListFrameMetrics.sanitizedCoordinate(-contentInset.top)
        let maxY = AetherListFrameMetrics.sanitizedCoordinate(
            AetherListFrameMetrics.sanitizedLength(contentSizeHeight)
                + contentInset.bottom
                - AetherListFrameMetrics.sanitizedLength(viewportHeight)
        )
        return AetherListFrameMetrics.sanitizedCoordinate(max(minY, maxY))
    }
    ...
    private static func compensatedContentOffset(
        ...
        let currentContentOffset = AetherListFrameMetrics.sanitizedPoint(currentContentOffset)
        let oldContentInset = AetherListFrameMetrics.sanitizedInsets(oldContentInset)
        let newContentInset = AetherListFrameMetrics.sanitizedInsets(newContentInset)
        let bottomAnchorTolerance = AetherListFrameMetrics.sanitizedLength(bottomAnchorTolerance)
        ...
        if alwaysCompensateBottomInsetChanges, abs(bottomDelta) > CGFloat.ulpOfOne {
            ...
            let offset = AetherListFrameMetrics.sanitizedPoint(CGPoint(x: currentContentOffset.x, y: targetY))
        ...
        let tolerance = bottomAnchorTolerance
        if currentContentOffset.y >= oldMaxY - tolerance {
            let anchoredOffset = AetherListFrameMetrics.sanitizedPoint(CGPoint(x: currentContentOffset.x, y: newMaxY))
        ...
        return AetherListFrameMetrics.sanitizedPoint(CGPoint(x: currentContentOffset.x, y: currentContentOffset.y - topDelta))
    }
...
public struct AetherListDebugCounters: Equatable {
    ...
    public internal(set) var updatedNodes: Int = 0
    public internal(set) var layoutApplications: Int = 0
    public internal(set) var preparedLayoutApplications: Int = 0
    public internal(set) var swipeRevealUpdates: Int = 0
    ...
    public internal(set) var maxTransactionDuration: TimeInterval = 0
    public internal(set) var lastNodeCreateDuration: TimeInterval = 0
    public internal(set) var totalNodeCreateDuration: TimeInterval = 0
    public internal(set) var lastNodeUpdateDuration: TimeInterval = 0
    public internal(set) var totalNodeUpdateDuration: TimeInterval = 0
    public internal(set) var lastLayoutApplicationDuration: TimeInterval = 0
    public internal(set) var totalLayoutApplicationDuration: TimeInterval = 0
    public internal(set) var lastPreparedLayoutApplicationDuration: TimeInterval = 0
    public internal(set) var totalPreparedLayoutApplicationDuration: TimeInterval = 0
    public internal(set) var lastSwipeRevealUpdateDuration: TimeInterval = 0
    public internal(set) var totalSwipeRevealUpdateDuration: TimeInterval = 0
}
...
public final class AetherListDebugInstrumentation {
    ...
    private let log = OSLog(subsystem: "AetherUI", category: "AetherListNode")
    ...
    internal func measure<T>(_ name: StaticString, _ body: () -> T) -> T {
        ...
        counters.lastTransactionDuration = duration
        counters.maxTransactionDuration = max(counters.maxTransactionDuration, duration)
        return result
    }
    ...
    @discardableResult
    internal func measureNodeCreate<T>(_ body: () -> T) -> T {
        counters.createdViews += 1
        return measureDuration(
            "AetherListNodeCreate",
            assign: { duration in
                self.counters.lastNodeCreateDuration = duration
                self.counters.totalNodeCreateDuration += duration
            },
            body
        )
    }
    ...
    @discardableResult
    internal func measureNodeUpdate<T>(_ body: () -> T) -> T {
        counters.updatedNodes += 1
        return measureDuration(
            "AetherListNodeUpdate",
            assign: { duration in
                self.counters.lastNodeUpdateDuration = duration
                self.counters.totalNodeUpdateDuration += duration
            },
            body
        )
    }
    ...
    @discardableResult
    internal func measureLayoutApplication<T>(_ body: () -> T) -> T {
        counters.layoutApplications += 1
        return measureDuration(
            "AetherListLayoutApply",
            assign: { duration in
                self.counters.lastLayoutApplicationDuration = duration
                self.counters.totalLayoutApplicationDuration += duration
            },
            body
        )
    }
    ...
    @discardableResult
    internal func measurePreparedLayoutApplication<T>(_ body: () -> T) -> T {
        counters.preparedLayoutApplications += 1
        return measureDuration(
            "AetherListPreparedLayoutApply",
            assign: { duration in
                self.counters.lastPreparedLayoutApplicationDuration = duration
                self.counters.totalPreparedLayoutApplicationDuration += duration
            },
            body
        )
    }
    ...
    @discardableResult
    internal func measureSwipeRevealUpdate<T>(_ body: () -> T) -> T {
        counters.swipeRevealUpdates += 1
        return measureDuration(
            "AetherListSwipeRevealUpdate",
            assign: { duration in
                self.counters.lastSwipeRevealUpdateDuration = duration
                self.counters.totalSwipeRevealUpdateDuration += duration
            },
            body
        )
    }
    ...
    @discardableResult
    private func measureDuration<T>(
        _ name: StaticString,
        assign: (TimeInterval) -> Void,
        _ body: () -> T
    ) -> T {
        ...
        let result = body()
        os_signpost(.end, log: log, name: name, signpostID: signpostID)
        assign(CACurrentMediaTime() - start)
        return result
    }
}
...
internal enum AetherListFrameMetrics {
    static func sanitizedCoordinate(_ value: CGFloat) -> CGFloat {
    ...
    static func sanitizedPoint(_ point: CGPoint) -> CGPoint {
        CGPoint(
            x: sanitizedCoordinate(point.x),
            y: sanitizedCoordinate(point.y)
        )
    }

    static func sanitizedInsets(_ insets: UIEdgeInsets) -> UIEdgeInsets {
        UIEdgeInsets(
            top: sanitizedCoordinate(insets.top),
            left: sanitizedCoordinate(insets.left),
            bottom: sanitizedCoordinate(insets.bottom),
            right: sanitizedCoordinate(insets.right)
        )
    }

    static func sanitizedRect(_ rect: CGRect) -> CGRect {
        CGRect(
            origin: sanitizedPoint(rect.origin),
            size: sanitizedSize(rect.size)
        )
    }
    ...
    static func pixelAligned(_ rect: CGRect, scale: CGFloat) -> CGRect {
        return CGRect(
            ...
            width: sanitizedLength(pixelAligned(rect.size.width, scale: scale)),
            height: sanitizedLength(pixelAligned(rect.size.height, scale: scale))
        )
    }
    ...
    static func rebuildOffsets(heights: [CGFloat], offsetInsets: UIEdgeInsets) -> (offsets: [CGFloat], totalHeight: CGFloat) {
        ...
        var y = sanitizedCoordinate(offsetInsets.top)
        for height in heights {
            offsets.append(sanitizedCoordinate(y))
            y += sanitizedLength(height)
            if !y.isFinite {
                y = 0.0
            }
        }
        return (offsets, sanitizedLength(y + sanitizedCoordinate(offsetInsets.bottom)))
    }
    ...
    static func scrollBounds(
        ...
        let contentInset = sanitizedInsets(contentInset)
        let contentSizeHeight = sanitizedLength(contentSizeHeight)
        let viewportHeight = sanitizedLength(viewportHeight)
        let minY = sanitizedCoordinate(-contentInset.top)
        let maxY = max(minY, sanitizedCoordinate(contentSizeHeight + contentInset.bottom - viewportHeight))
        return (minY, maxY, sanitizedLength(maxY - minY))
    }
    ...
    static func pagedScrollToTopOffset(
        ...
        let currentOffsetY = sanitizedCoordinate(currentOffsetY)
        let viewportHeight = sanitizedLength(viewportHeight)
        let contentInset = sanitizedInsets(contentInset)
        ...
        let defaultDistance = sanitizedLength(visiblePageHeight * 2.0)
        let pageDistance = max(1.0, sanitizedLength(distance ?? defaultDistance))
        ...
        return sanitizedCoordinate(targetY)
    }
    ...
    static func overscrollDistances(
        ...
        let contentOffsetY = sanitizedCoordinate(contentOffsetY)
        return (
            top: sanitizedLength(bounds.minY - contentOffsetY),
            bottom: sanitizedLength(contentOffsetY - bounds.maxY)
        )
    }
    ...
    static func verticalScrollIndicatorFrame(
        ...
        let boundsWidth = sanitizedLength(boundsWidth)
        let viewportHeight = sanitizedLength(viewportHeight)
        let contentInset = sanitizedInsets(contentInset)
        let scrollIndicatorInsets = scrollIndicatorInsets.map(sanitizedInsets)
        let contentOffsetY = sanitizedCoordinate(contentOffsetY)
        let indicatorWidth = sanitizedLength(indicatorWidth)
        let rightInset = sanitizedLength(rightInset)
        let minimumThumbHeight = sanitizedLength(minimumThumbHeight)
    ...
    static func visibleRange(
        ...
        let viewportTop = sanitizedCoordinate(viewportTop)
        let viewportHeight = sanitizedLength(viewportHeight)
        let preload = sanitizedLength(preloadPages) * viewportHeight
        ...
        while lo <= hi {
            let mid = (lo + hi) / 2
            let itemBottom = sanitizedCoordinate(offsets[mid]) + sanitizedLength(heights[mid])
            if itemBottom < top {
                lo = mid + 1
            } else {
                hi = mid - 1
        ...
        while lo <= hi {
            let mid = (lo + hi) / 2
            let itemTop = sanitizedCoordinate(offsets[mid])
            if itemTop > bottom {
                hi = mid - 1
            } else {
                lo = mid + 1
    ...
    static func scrollOffset(
        ...
        let nodeInsets = sanitizedInsets(nodeInsets)
        let insets = sanitizedInsets(insets)
        let viewportHeight = sanitizedLength(viewportHeight)
        let currentOffset = sanitizedCoordinate(currentOffset)
        let itemTop = sanitizedCoordinate(offsets[index]) + nodeInsets.top
        let itemHeight = sanitizedLength(heights[index] - nodeInsets.top - nodeInsets.bottom)
        ...
        case .visible:
            ...
            if itemTop < visibleTop {
                resolvedOffset = itemTop - insets.top
                break
            }
            resolvedOffset = itemBottom - viewportHeight + insets.bottom

        case .top(let offset):
            resolvedOffset = itemTop - insets.top - sanitizedCoordinate(offset)

        case .bottom(let offset):
            resolvedOffset = itemBottom - viewportHeight + insets.bottom + sanitizedCoordinate(offset)

        case .center:
            resolvedOffset = itemTop + itemHeight / 2 - viewportHeight / 2
        ...
        case .centerWithOverflow(let overflow):
            ...
            case .bottom:
                resolvedOffset = itemBottom - viewportHeight + insets.bottom
            case .custom:
                resolvedOffset = customOverflow.map {
                    itemBottom - viewportHeight + insets.bottom + sanitizedCoordinate($0)
                } ?? (itemTop - insets.top)
            }
        }
        return sanitizedCoordinate(resolvedOffset)
    }
