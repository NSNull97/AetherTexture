...
/// Protocol for items displayed in a `AetherListNode`.
///
/// Each item knows how to create and update its node. The list node manages
/// lifecycle, reuse, and layout.
public protocol AetherListItem: AnyObject {
    ...
    /// accessories. The Texture list does not force a concrete accessory
    ...
    /// unless `AetherListNode.allowsReorder == true`.
...
public extension AetherListItem {
    ...
    func swipeActionSelected(_ action: AetherListSwipeAction, listNode: AetherListNode, isFullSwipe: Bool) {}
    func selected(listNode: AetherListNode) {}
    func performSecondaryAction(listNode: AetherListNode) {}
...
public struct AetherListItemNodeLayout {
    ...
    public init(contentSize: CGSize, insets: UIEdgeInsets = .zero) {
        self.contentSize = AetherListFrameMetrics.sanitizedSize(contentSize)
        self.insets = AetherListFrameMetrics.sanitizedInsets(insets)
    }

    /// Total height including insets.
    public var totalHeight: CGFloat {
        return AetherListFrameMetrics.sanitizedLength(insets.top + contentSize.height + insets.bottom)
    }
}
...
/// Selection mode for a `AetherListNode`. `.none` falls back to the
/// classic "tap fires `item.selected(listNode:)`" behaviour. The two
/// active modes drive `node.isSelected` based on tap events and emit
/// `AetherListNode.selectionChanged`.
