...
private var aetherSwipeTitleFont: UIFont {
    UIFont.aetherScaledSystemFont(ofSize: 11.0, weight: .regular, maximumPointSize: 14.0)
}
private var aetherSwipeIconlessTitleFont: UIFont {
    UIFont.aetherScaledSystemFont(ofSize: 13.0, weight: .regular, maximumPointSize: 17.0)
}
...
internal final class AetherListSwipeOptionContainer: UIView {
    private final class OptionView: UIView {
        ...
        private let iconImageSize: CGSize?
        private let titleAttributedText: NSAttributedString
        ...
        init(action: AetherListSwipeAction) {
            ...
            case let .image(image):
                ...
                iconNode = imageNode
                iconImageSize = image.size
            }

            displaysTitleInsidePill = !action.icon.hasVisualIcon
            titleAttributedText = Self.attributedTitle(
                action.title,
                font: displaysTitleInsidePill ? aetherSwipeIconlessTitleFont : aetherSwipeTitleFont,
                color: displaysTitleInsidePill ? action.foregroundColor : (action.textColor ?? .secondaryLabel)
            )
            ...
            contentContainerNode.layer.allowsGroupOpacity = true
            addSubview(contentContainerNode.view)

            backgroundNode.backgroundColor = action.backgroundColor
            backgroundNode.layer.masksToBounds = true
            contentContainerNode.addSubnode(backgroundNode)

            titleNode.attributedText = titleAttributedText
            titleNode.maximumNumberOfLines = 1
            contentContainerNode.addSubnode(titleNode)

            if let iconNode {
                contentContainerNode.addSubnode(iconNode)
            }
        }
        ...
        func updateLayout(
            ...
            contentContainerNode.view.bounds = bounds
            contentContainerNode.view.center = CGPoint(x: bounds.midX, y: bounds.midY)

            let titleSize = Self.titleSize(titleAttributedText, constrainedWidth: metrics.titleWidth)
            ...
            transition.updateFrame(view: backgroundNode.view, frame: backgroundFrame)
            backgroundNode.layer.cornerRadius = metrics.cornerRadius
            ...
            transition.updateAlpha(view: contentContainerNode.view, alpha: contentAlpha)
            transition.updateTransform(view: contentContainerNode.view, transform: CGAffineTransform(scaleX: contentScale, y: contentScale))
            ...
            if let iconNode, let imageSize = iconImageSize {
                ...
                transition.updateFrame(view: iconNode.view, frame: iconFrame)
            }

            let titleAlpha: CGFloat = isPrimary && !displaysTitleInsidePill ? (1.0 - expandedProgress) : 1.0
            transition.updateAlpha(view: titleNode.view, alpha: titleAlpha)
            ...
            transition.updateFrame(view: titleNode.view, frame: titleFrame)
        ...
        private static func titleSize(_ attributedTitle: NSAttributedString, constrainedWidth: CGFloat) -> CGSize {
