...
open class TextNode: ASDisplayNode, UIGestureRecognizerDelegate {
    public static var defaultFont: UIFont {
        UIFont.aetherScaledSystemFont(ofSize: 15.0)
    }
    ...
    private let textDisplayNode = AetherTextDisplayNode()
    ...
    open override func calculateSizeThatFits(_ constrainedSize: CGSize) -> CGSize {
        return updateLayout(constrainedSize).size
    }
    ...
    open override func layout() {
        ...
        textDisplayNode.frame = CGRect(origin: .zero, size: bounds.size)
        updateTextDisplayNodeFrame(for: bounds.size)
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        guard let point = touches.first?.location(in: view), let hit = interactiveHit(at: point) else {
            return
        }
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        guard let activeHit, let point = touches.first?.location(in: view) else {
            return
        }
    ...
    open override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        return interactiveHit(at: gestureRecognizer.location(in: view)) != nil
    }
    ...
    private func relayoutText() {
        ...
        textContainer.lineBreakMode = lineBreakMode
        textDisplayNode.update(
            attributedText: textStorage.length > 0 ? NSAttributedString(attributedString: textStorage) : nil,
            maximumNumberOfLines: maximumNumberOfLines,
            truncationMode: lineBreakMode
        )
    ...
    private func ensureLayout(for size: CGSize) {
        let width = max(0.0, finiteDimension(size.width, fallback: 10_000.0) - insets.left - insets.right)
    ...
    private func updateTextDisplayNodeFrame(for size: CGSize) {
        let width = max(0.0, finiteDimension(size.width, fallback: 10_000.0) - insets.left - insets.right)
        let height = max(0.0, finiteDimension(size.height, fallback: CGFloat.greatestFiniteMagnitude / 4.0) - insets.top - insets.bottom)
        let origin = textDrawingOrigin(for: size)
        textDisplayNode.updateTextFrame(CGRect(x: origin.x, y: origin.y, width: width, height: height))
    }
    ...
    @objc private func handleLongPress(_ recognizer: UILongPressGestureRecognizer) {
        ...
        case .began:
            guard let hit = interactiveHit(at: recognizer.location(in: view)) else {
                return
            }
            activeHit = hit
