...
final class AetherModalPresentationController: UIPresentationController, UIGestureRecognizerDelegate {
    ...
    override func presentationTransitionWillBegin() {
        ...
        let targetDimAlpha = dimAlpha(forProgress: detent == .stage2 ? 1.0 : 0.0)
        if let coordinator = presentedViewController.transitionCoordinator {
            coordinator.animate(alongsideTransition: { [weak self] _ in
                self?.dimView.alpha = targetDimAlpha
            })
        } else {
            dimView.alpha = targetDimAlpha
        }
    }
    ...
    private func frame(for detent: AetherModalController.Detent, in bounds: CGRect) -> CGRect {
        ...
        switch detent {
        case .stage1:
            let sideInset = min(max(0.0, cfg.sideInset), bounds.width / 2.0)
            let bottom = max(safeArea.top, bounds.height - max(0.0, cfg.bottomInsetStage1))
            let resolvedTopInset = cfg.topInsetStage1 < 0.0 ? bounds.height * 0.5 : cfg.topInsetStage1
            let configuredTop = safeArea.top + max(0.0, resolvedTopInset)
            let minimumHeight = bounds.height * 0.5
            let height = min(bottom - safeArea.top, max(bottom - configuredTop, minimumHeight))
            let top = bottom - height
            return CGRect(
                x: sideInset,
                y: top,
                width: max(0.0, bounds.width - sideInset * 2.0),
                height: max(0.0, height)
            )
        case .stage2:
            let bottom = max(safeArea.top, bounds.height - max(0.0, cfg.bottomInsetStage2))
            let top = min(bottom, safeArea.top + max(0.0, cfg.topInsetStage2))
            return CGRect(
                x: 0.0,
                y: top,
                width: bounds.width,
    ...
    @objc private func handlePan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            // Follow the recognizer directly. UIPanGestureRecognizer is
            // already coalesced to display updates by UIKit; feeding it
            // through another CADisplayLink made the sheet chase the finger
            // and kept scheduling expensive frame updates after the input
            // itself had stopped.
            if dragDriving {
                applyDrag(
        ...
        case .ended, .cancelled, .failed:
            ...
            applyDrag(
                translation: translation,
                container: container,
                draggingDown: translation.y > 0,
                draggingUp: translation.y < 0
            )
    ...
    private func animateTo(
        ...
        // Keep enough time for the destination to read as a real detent.
        // The old 0.25/0.35 envelope could then be halved for a partial
        // drag, reducing the entire settle to 0.125s. That made the sheet
        // appear to teleport as soon as the finger lifted.
        let baseDuration: CFTimeInterval = isCollapsingTowardStage1 ? 0.46 : 0.50
        ...
        let distanceFraction = fullDistance > 0
            ? max(0.0, min(1.0, distance / fullDistance))
            : 1.0
        // A short correction may be shorter than a full transition, but
        // never so short that the detent snap becomes a flash. Velocity
        // only trims the tail slightly; it no longer halves the motion.
        let distanceScale = 0.74 + 0.26 * CFTimeInterval(distanceFraction)
        let velocityFraction = min(1.0, abs(initialVelocityY) / 2400.0)
        let velocityScale = 1.0 - 0.12 * CFTimeInterval(velocityFraction)
        ...
        settleDuration = max(0.34, baseDuration * distanceScale * velocityScale)
    ...
    private func tickSettleLink() {
        ...
        // A gentle spring is reserved for iOS 26+.
        ...
        if #available(iOS 26.0, *) {
            t = Self.easeOutSpring(raw)
        } else {
            t = Self.easeOutCubic(raw)
        }
        let f = CGRect(
    ...
    /// Lightly underdamped spring step response with only a subtle tail.
    /// It deliberately advances more gradually than the old 0.62 / 9.5
    /// curve, which had already covered almost 90% of the distance after
    /// the first quarter of its duration.
    private static func easeOutSpring(_ t: CGFloat) -> CGFloat {
        if t <= 0 { return 0 }
        if t >= 1 { return 1 }
        let zeta: CGFloat = 0.85
