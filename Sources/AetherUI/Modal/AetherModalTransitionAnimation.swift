...
/// Internal lifecycle hook for built-in transition implementations that own
/// Liquid-only renderers or animation drivers.
protocol AetherModalLiquidTransitionStateControlling: AnyObject {
    func aetherModalCancelLiquidTransitionState()
}
...
public final class AetherModalSourceTransition: AetherModalTransitionAnimation,
    AetherModalLiquidTransitionStateControlling {
    public struct Configuration: Equatable {
        ...
        public var dismissalInitialSpringVelocity: CGFloat
        ...
        public init(
            ...
            presentationDampingRatio: CGFloat = AetherMotion.sourceModal.presentation.dampingRatio,
            dismissalDampingRatio: CGFloat = AetherMotion.sourceModal.dismissal.dampingRatio,
            initialSpringVelocity: CGFloat = AetherMotion.sourceModal.presentation.initialVelocity,
            dismissalInitialSpringVelocity: CGFloat = AetherMotion.sourceModal.dismissal.initialVelocity,
            overscaleAmount: CGFloat = 0.018,
            ...
            self.dismissalInitialSpringVelocity = dismissalInitialSpringVelocity
    ...
    private weak var activeMorphView: AetherModalSourceMorphTransitionView?
    ...
    public func makePresentationAnimator(
        ...
        guard modalController.resolvedAppearanceStyleForModalTransition.usesLiquidGlass else {
            return nil
        }
        return AetherModalSourcePresentAnimator(
    ...
    public func makeDismissalAnimator(
        ...
        return AetherModalSourceDismissAnimator(
            sourceTransition: self,
            modalController: modalController
        )
    }

    fileprivate func setActiveMorphView(_ morphView: AetherModalSourceMorphTransitionView) {
        activeMorphView = morphView
    }

    fileprivate func clearActiveMorphView(_ morphView: AetherModalSourceMorphTransitionView) {
        if activeMorphView === morphView {
            activeMorphView = nil
        }
    }

    func aetherModalCancelLiquidTransitionState() {
        activeMorphView?.finishImmediately()
        activeMorphView = nil
    }

    internal var hasActiveLiquidTransitionStateForTesting: Bool {
        activeMorphView != nil
    }
...
private final class AetherModalSourcePresentAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using ctx: UIViewControllerContextTransitioning) {
        guard modalController?.resolvedAppearanceStyleForModalTransition.usesLiquidGlass == true else {
        ...
        sourceTransition.setActiveMorphView(morphView)
        ...
        morphView.animateExpand(
            ...
            completion: {
                self.sourceTransition.clearActiveMorphView(morphView)
...
private final class AetherModalSourceDismissAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using ctx: UIViewControllerContextTransitioning) {
        ...
        let sourceAlpha = sourceView?.alpha
        if cfg.hidesSourceViewDuringTransition {
            // Keep a single visual owner during the reverse morph. The source
            // snapshot inside `morphView` is visible until collapse reaches its
            // endpoint; showing the real source early creates a duplicate and
            // a bright flash under transparent glass.
            sourceView?.alpha = 0.0
        }
        ...
        morphView.animateCollapse(
            ...
            initialVelocity: cfg.dismissalInitialSpringVelocity,
            overscaleAmount: cfg.overscaleAmount,
            completion: {
                self.sourceTransition.clearActiveMorphView(morphView)
                let completed = !ctx.transitionWasCancelled
...
private final class AetherModalSourceMorphTransitionView: UIView {
    ...
    private let destinationBlurView = UIVisualEffectView(effect: UIBlurEffect(style: .systemUltraThinMaterial))
    ...
    init() {
        ...
        destinationBlurView.isUserInteractionEnabled = false
        destinationContent.addSubview(destinationBlurView)
    ...
    func installDestinationSnapshot(_ snapshot: UIView) {
        destinationContent.subviews
            .filter { $0 !== destinationBlurView }
            .forEach { $0.removeFromSuperview() }
        snapshot.frame = destinationContent.bounds
        snapshot.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        destinationContent.insertSubview(snapshot, belowSubview: destinationBlurView)
        destinationContent.bringSubviewToFront(destinationBlurView)
    }
    ...
    func animateExpand(
        ...
        animateShadow(fromExpanded: false, toExpanded: true, duration: duration)
        animateProgress(
            to: 1.0,
            duration: duration,
            damping: damping,
    ...
    func animateCollapse(
        ...
        isCollapsing = true
        progressOvershootLimit = Self.resolvedOvershootLimit(overscaleAmount)
        animateShadow(fromExpanded: true, toExpanded: false, duration: duration)
        animateProgress(
            to: 0.0,
            duration: duration,
            damping: damping,
    ...
    private func animateProgress(
        ...
        let directedVelocity = reversed ? -abs(initialVelocity) : abs(initialVelocity)
        let timing = UISpringTimingParameters(
            dampingRatio: max(0.01, min(1.0, damping)),
            initialVelocity: CGVector(dx: directedVelocity, dy: 0.0)
        )
    ...
    private func configureContentFrames(metrics: Metrics) {
        ...
        destinationBlurView.frame = destinationContent.bounds
        destinationBlurView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
    }
    ...
    private func startProgressDisplayLink() {
        ...
        let maximumFramesPerSecond = max(60, window?.screen.maximumFramesPerSecond ?? UIScreen.main.maximumFramesPerSecond)
    ...
    private func applyProgress(_ value: CGFloat) {
        ...
        let transientSurface = sin(.pi * t)
        materialView.alpha = 0.42 * transientSurface
        tintView.backgroundColor = UIColor.white.withAlphaComponent(0.08 * transientSurface)
        highlightView.alpha = 0.32 * transientSurface
        ...
        let sourceAlpha = isCollapsing
            ...
            : 1.0 - Self.smootherstep(0.02, 0.18, t)
        let destinationAlpha = isCollapsing
            ? Self.smootherstep(0.10, 0.58, t)
            : Self.smootherstep(0.14, 0.70, t)
        ...
        destinationContent.transform = CGAffineTransform(
            translationX: 0.0,
            y: isCollapsing ? 0.0 : Self.lerp(10.0, 0.0, Self.smootherstep(0.14, 0.72, t))
        )
        destinationBlurView.alpha = 1.0 - Self.smootherstep(0.46, 0.92, t)
        CATransaction.commit()
    }
    ...
    /// Synchronously settles the in-flight UIKit transition and tears down its
    /// display-link/property-animator state. Used when a live appearance switch
    /// selects Legacy while the Liquid source morph is running.
    func finishImmediately() {
        guard let completion = progressCompletion else {
            cancelRunningAnimators()
            removeFromSuperview()
            return
        }
        ...
        let endpoint: CGFloat = isCollapsing ? 0.0 : 1.0
        progressCompletion = nil
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        progressDriverView.layer.removeAllAnimations()
        progress = endpoint
        progressDriverView.transform = CGAffineTransform(
            translationX: endpoint,
            y: 0.0
        )
        applyProgress(endpoint)
        CATransaction.commit()
        completion()
    }
}
