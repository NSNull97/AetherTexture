...
/// Direct bridge to the optional keyboard runtime. Keeping this as a C entry
/// point avoids relying on Objective-C category discovery when AetherUI is
/// linked as a static SwiftPM product.
UIWindow * _Nullable AetherLegacyKeyboardWindow(void);

/// Linker probe used by tests to verify that the Objective-C runtime bridge is
/// part of the final product even when its result is currently `nil`.
